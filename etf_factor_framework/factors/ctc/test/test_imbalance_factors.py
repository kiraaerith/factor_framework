...
def test_vol_amplitude_imbalance():
    """测试振幅不平衡度因子"""
    print("="*60)
    print("测试1: VolAmplitudeImbalance因子基础功能")
    print("="*60)
    ...
    print("\n[PASS] VolAmplitudeImbalance基础功能测试通过")


def test_vol_return_std_imbalance():
    """测试收益率标准差不平衡度因子"""
    print("\n" + "="*60)
    print("测试2: VolReturnStdImbalance因子基础功能")
    print("="*60)
    ...
    print("\n[PASS] VolReturnStdImbalance基础功能测试通过")
...
def test_with_real_data():
    ...
    # 测试VolAmplitudeImbalance
    print("\n测试 VolAmplitudeImbalance...")
    factor1 = VolAmplitudeImbalance(window=20, top_pct=0.2)
    factor_data1 = factor1.calculate(ohlcv)
    print(f"因子计算完成，形状: {factor_data1.shape}")
    print(f"因子值统计:")
    print(f"  - 均值: {factor_data1.values.mean().mean():.4f}")
    print(f"  - 标准差: {factor_data1.values.std().mean():.4f}")
    print(f"  - 最小值: {factor_data1.values.min().min():.4f}")
    print(f"  - 最大值: {factor_data1.values.max().max():.4f}")
    
    # 测试VolReturnStdImbalance
    print("\n测试 VolReturnStdImbalance...")
    factor2 = VolReturnStdImbalance(window=20, top_pct=0.2)
    factor_data2 = factor2.calculate(ohlcv)
    print(f"因子计算完成，形状: {factor_data2.shape}")
    print(f"因子值统计:")
    print(f"  - 均值: {factor_data2.values.mean().mean():.4f}")
    print(f"  - 标准差: {factor_data2.values.std().mean():.4f}")
    print(f"  - 最小值: {factor_data2.values.min().min():.4f}")
    print(f"  - 最大值: {factor_data2.values.max().max():.4f}")
    
    print("\n[PASS] 真实数据测试通过")


if __name__ == '__main__':
    test_vol_amplitude_imbalance()
    test_vol_return_std_imbalance()
    test_with_real_data()
    print("\n" + "="*60)
    print("所有测试通过！")
    print("="*60)
