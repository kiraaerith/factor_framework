...
def test_factor_basic():
    """测试因子基础功能"""
    print("="*60)
    print("测试1: VolReturnStdImbalance 因子基础功能")
    print("="*60)
    ...
    # 打印前5个非NaN值作为示例
    valid_values = factor_data.values.values.flatten()
    ...
    # 检查输出格式
    print(f"因子数据形状: {factor_data.shape}, OHLCV形状: {ohlcv.shape}")
    assert factor_data.shape[0] == ohlcv.n_assets, f"资产数量不匹配: {factor_data.shape[0]} vs {ohlcv.n_assets}"
    assert factor_data.shape[1] == ohlcv.n_periods, f"时间长度不匹配: {factor_data.shape[1]} vs {ohlcv.n_periods}"
