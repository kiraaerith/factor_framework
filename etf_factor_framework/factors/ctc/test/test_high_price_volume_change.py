"""
HighPriceVolumeChange 因子单元测试脚本
"""
...
def test_factor_basic():
    """测试因子基础功能"""
    print("="*60)
    print("测试1: HighPriceVolumeChange 因子基础功能")
    print("="*60)
    ...
    # 检查输出格式
    assert factor_data.shape[:2] == ohlcv.shape[:2], f"因子形状与数据不匹配: {factor_data.shape} vs {ohlcv.shape}"
...
def test_different_params():
    ...
    ohlcv = create_test_data(n_assets=3, n_periods=40)
    
    params_list = [
        {'window': 10, 'top_pct': 0.1},
        {'window': 20, 'top_pct': 0.2},
        {'window': 30, 'top_pct': 0.3},
    ]
    
    for params in params_list:
        factor = HighPriceVolumeChange(**params)
        factor_data = factor.calculate(ohlcv)
        
        print(f"\n参数: window={params['window']}, top_pct={params['top_pct']}")
        print(f"  名称: {factor.name}")
        print(f"  有效值数量: {factor_data.values.notna().sum().sum()}")
