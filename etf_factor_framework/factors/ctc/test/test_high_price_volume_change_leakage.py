"""
HighPriceVolumeChange 因子数据泄露检测脚本
"""
...
def main():
    ...
    # 方法1: 使用便捷函数检测
    print("\n" + "="*60)
    print("Data Leakage Detection (Using detect_leakage function)")
    print("="*60)
    report = detect_leakage(factor, ohlcv, split_ratio=0.5, verbose=True)
    
    # 方法2: 使用检测器类（更灵活）
    print("\n" + "="*60)
    print("Data Leakage Detection (Using LeakageDetector class)")
    print("="*60)
    ...
    if report.has_leakage:
        ...
        print(f"PASSED: {factor.name} has NO data leakage, can proceed with evaluation.")
        return True
