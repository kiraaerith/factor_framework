...
数据字段：lixinger financial_statements.q_m_roa_t
因子方向：正向（ROA 越高越好）
因子类别：盈利能力
...
class ROA(FundamentalFactorCalculator):
    """
    总资产收益率（ROA）因子

    使用 lixinger 的 q_m_roa_t（季度 ROA，时间加权），
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Returns:
            FactorData: N股 × T日，值为 q_m_roa_t（百分比，如 0.10 = 10%）
        """
        values, symbols, dates = fundamental_data.get_daily_panel(FIELD)

        if values.size == 0:
            raise ValueError(f"ROA 因子：get_daily_panel('{FIELD}') 返回空数组，"
                             "请检查 lixinger 数据库中是否有该字段数据")

        nan_ratio = np.isnan(values).sum() / values.size
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(f"ROA 因子 NaN 比例偏高：{nan_ratio:.1%}，请检查数据")
