"""
Engineering tests for ROE factor.
Run from etf_factor_framework directory:
    python factors/fundamental/profitability/test_roe_engineering.py
"""
import os
import sys
import sqlite3
...
# ----------------------------------------------------------------
# Test 4: Golden Test
# q_m_roe_t is a direct field from DB - compare factor value vs DB value
...
df_gold = pd.read_sql(
    f"SELECT {code_col}, date, report_date, q_m_roe_t FROM financial_statements "
...
expected_val = float(df_gold['q_m_roe_t'].iloc[0])
release_date_str = pd.Timestamp(df_gold['report_date'].iloc[0]).strftime('%Y-%m-%d')
print(f"  Expected q_m_roe_t = {expected_val}, released on {release_date_str}")
...
# q_m_roe_t is a quarterly TTM field.
...
if len(q3_rows) > 0 and len(q2_rows) > 0:
    ...
    expected_before = float(q2_rows.iloc[0]['q_m_roe_t'])
    expected_after  = float(q3_rows.iloc[0]['q_m_roe_t'])
