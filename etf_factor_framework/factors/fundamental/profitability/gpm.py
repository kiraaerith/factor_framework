...
数据字段：lixinger financial_statements.q_ps_gp_m_t
因子方向：正向（毛利率越高越好）
因子类别：盈利能力 - 利润率
...
class GPM(FundamentalFactorCalculator):
    """
    毛利率（Gross Profit Margin）因子

    使用 lixinger 的 q_ps_gp_m_t（季度利润表毛利率，TTM），
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Returns:
            FactorData: N股 × T日，值为 q_ps_gp_m_t（小数形式，如 0.30 = 30%）
        """
        values, symbols, dates = fundamental_data.get_daily_panel(FIELD)

        if values.size == 0:
            raise ValueError(f"GPM 因子：get_daily_panel('{FIELD}') 返回空数组，"
                             "请检查 lixinger 数据库中是否有该字段数据")

        nan_ratio = np.isnan(values).sum() / values.size
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(f"GPM 因子 NaN 比例偏高：{nan_ratio:.1%}，请检查数据")
