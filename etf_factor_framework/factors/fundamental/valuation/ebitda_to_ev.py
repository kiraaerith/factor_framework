"""
EBITDA_TO_EV 因子（企业价值盈利收益率：EBITDA/EV）

EV/EBITDA 是国际常用的跨行业估值指标，剔除了资本结构和折旧摊销差异。
本因子取其倒数 EBITDA/EV，使方向为正向（值越大 = 企业越便宜 = 越好）。

Formula:
    EBITDA_TO_EV = 1 / ev_ebitda_r

Data fields from lixinger.fundamental (daily valuation):
    - ev_ebitda_r : EV/EBITDA，由 lixinger 日频计算，非空率 83.1%

Factor direction: +1 (higher EBITDA/EV → cheaper enterprise → more desirable)
Factor category: valuation

Notes:
    - ev_ebitda_r comes from the daily fundamental table (no quarterly lag).
      T-day value is known as of T-day close → no future data leakage.
    - When ev_ebitda_r ≤ 0 (negative EBITDA) the ratio is economically
      meaningless as a value signal; those cells are set to NaN.
    - Inf values (ev_ebitda_r ≈ 0) are also set to NaN.
...
FIELD = "ev_ebitda_r"       # EV/EBITDA ratio, from lixinger.fundamental (daily)
FACTOR_NAME = "EBITDA_TO_EV"
FACTOR_DIRECTION = 1        # +1: higher EBITDA/EV → cheaper → better

MC_MIN_BILLION = 50.0       # 市值下限（亿元），与 piotroski_fscore.py 保持一致


class EbitdaToEv(FundamentalFactorCalculator):
    """
    EBITDA/EV 因子（企业价值盈利收益率）

    使用 lixinger fundamental.ev_ebitda_r（日频 EV/EBITDA），取倒数得到
    EBITDA/EV。负值（EBITDA < 0）和 inf 值置 NaN。
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Returns
        -------
        FactorData : shape (N, T)，值为 1 / ev_ebitda_r；
                     ev_ebitda_r ≤ 0 或市值 < MC_MIN_BILLION 的股票-日置 NaN。
        """
        # ------------------------------------------------------------------
        # Step 1: Load EV/EBITDA panel from daily fundamental table
        # ------------------------------------------------------------------
        try:
            values, symbols, dates = fundamental_data.get_valuation_panel(FIELD)
        except Exception as exc:
            raise ValueError(
                f"EbitdaToEv: failed to load field '{FIELD}': {exc}"
            ) from exc

        if values.size == 0:
            raise ValueError(
                f"EbitdaToEv: get_valuation_panel('{FIELD}') returned empty array. "
        ...
        #   ev_ebitda_r ≤ 0 means negative EBITDA → not a valid value signal
        # ------------------------------------------------------------------
        ...
        # Mask any remaining inf (ev_ebitda_r extremely close to 0)
        ...
        try:
            ...
            warnings.warn(
                f"EbitdaToEv: market-cap filter skipped due to error: {exc}. "
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"EBITDA_TO_EV NaN ratio is very high ({nan_ratio:.1%}). "
                "Check if field 'ev_ebitda_r' is populated in the lixinger database."
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python ebitda_to_ev.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("EBITDA_TO_EV factor smoke test")
    print("=" * 60)
    ...
    print("\n[Step 2] Calculate EBITDA_TO_EV factor")
    calculator = EbitdaToEv()
