"""
AD_TO_COST factor (Sales Expense / Advertising Proxy to Period Cost Ratio, TTM)

Formula: SE_TTM / TotalCost_TTM
  - SE_TTM:        TTM rolling sum of q_ps_se_c (single-quarter sales expense)
                   Used as proxy for advertising expense (no separate ad field in lixinger)
...
TotalCost_TTM = period expenses (sales + admin + finance expenses), i.e.
    total operating cost minus direct cost of goods sold.

Note: lixinger has no separate advertising expense field. Sales expense (q_ps_se_c)
is used as the best proxy for advertising/promotion spend.
...
Factor direction: positive (higher sales/ad expense share of period costs predicts
                            better brand investment; marketing-driven allocation signal)
Factor category: growth - advertising investment

Boundary conditions:
  - TotalCost_TTM <= 0  -> NaN  (anomalous or zero period cost; ratio direction invalid)
  - SE_TTM is NaN       -> NaN  (company did not disclose sales expense; missing = info)
  - SE_TTM < 0          -> NaN  (data anomaly; sales expense should not be negative)
  - toc_TTM or oc_TTM NaN -> TotalCost_TTM = NaN -> ratio = NaN
  - AD_to_TotalCost > 1 -> kept (extreme but possible; downstream Winsorize handles it)
"""
...
FACTOR_NAME = "AD_TO_COST"
FACTOR_DIRECTION = 1  # positive: higher sales/ad share of period costs is better


class AD_TO_COST(FundamentalFactorCalculator):
    """
    Sales Expense (Advertising Proxy) to Period Cost Ratio (TTM).

    AD_TO_COST = SE_TTM / TotalCost_TTM
    where:
      SE_TTM        = rolling 4-quarter sum of q_ps_se_c (sales expense, single-quarter)
      TotalCost_TTM = rolling 4-quarter sum of (q_ps_toc_c - q_ps_oc_c)
                    = period expenses TTM (total operating cost minus COGS)

    Companies not disclosing sales expense (NaN q_ps_se_c) receive NaN factor values.
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Compute AD_TO_COST daily panel (N stocks x T days).

        Returns:
            FactorData with values = SE_TTM / TotalCost_TTM (dimensionless ratio)
        """
        ...
        se_v, se_s, _ = self._compute_ttm_panel(
        ...
        if se_v.size == 0:
            raise ValueError("AD_TO_COST: q_ps_se_c TTM panel is empty")
        if toc_v.size == 0:
            raise ValueError("AD_TO_COST: q_ps_toc_c TTM panel is empty")
        if oc_v.size == 0:
            raise ValueError("AD_TO_COST: q_ps_oc_c TTM panel is empty")

        # --- 2. Align to common symbols ---
        common_syms = np.intersect1d(np.intersect1d(se_s, toc_s), oc_s)
        if len(common_syms) == 0:
            raise ValueError("AD_TO_COST: no common symbols across SE / toc / oc panels")

        se_aligned  = se_v[np.searchsorted(se_s, common_syms)]
        ...
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(
                f"AD_TO_COST NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python ad_to_cost.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("AD_TO_COST factor smoke test")
    print("=" * 60)
