"""
AD_TO_MV factor (Selling Expense to Market Cap Ratio, TTM)

Formula: SE_TTM / MarketCap
...
Note on naming: lixinger does not store advertising expense as a standalone field.
Selling expense (q_ps_se_c) is used as the best available proxy for advertising expense.
The factor is thus "selling expense to market cap ratio (TTM)".

SE_TTM is computed from lixinger.financial_statements; TTM = rolling sum of 4 most
...
Factor direction: positive (higher selling-expense-to-market-cap ratio means market
                            underprices marketing investment; analogous to RD_TO_MV)
Factor category: growth - advertising/marketing investment

Unit alignment: both q_ps_se_c and mc are in yuan (CNY), direct division is valid.

Boundary conditions:
  - MarketCap <= 0  -> NaN  (market cap should be strictly positive)
  - SE_TTM is NaN   -> NaN  (company did not disclose selling expense; missing = info)
  - SE_TTM < 0      -> NaN  (data anomaly; selling expense should not be negative)
  - AD_to_MV > 1   -> kept (extreme but rare; downstream Winsorize handles it)
"""
...
FACTOR_NAME = "AD_TO_MV"
FACTOR_DIRECTION = 1  # positive: higher selling-expense-to-market-cap ratio is better
...
class AD_TO_MV(FundamentalFactorCalculator):
    """
    Selling Expense (proxy for Advertising) to Market Cap Ratio (TTM).

    AD_TO_MV = SE_TTM / MarketCap
    where:
      SE_TTM    = rolling 4-quarter sum of q_ps_se_c (selling expense, single-quarter, yuan)
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Compute AD_TO_MV daily panel (N stocks x T days).

        Returns:
            FactorData with values = SE_TTM / MarketCap (dimensionless ratio)
        """
        ...
        if len(common_syms) == 0:
            raise ValueError("AD_TO_MV: no common mainboard symbols between SE and MarketCap panels")

        se_aligned = se_v[np.searchsorted(se_s, common_syms)]   # (N, T)
        ...
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(
                f"AD_TO_MV NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python ad_to_mv.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("AD_TO_MV factor smoke test")
    print("=" * 60)
    ...
    v, s, d = fd.get_daily_panel("q_ps_se_c")
    nr = np.isnan(v).mean()
    print(f"  q_ps_se_c: shape={v.shape}, NaN={nr:.1%}")
    assert v.size > 0, "q_ps_se_c returned empty array"
    ...
    print(f"\n[Step 3] Compute AD_TO_MV factor")
    calculator = AD_TO_MV()
