"""
growth_NP_REV factor (Net Profit + Revenue Combined Growth Composite)

Combines 16 sub-factors across two dimensions:
  NP dimension (attributable net profit, q_ps_npatoshopc_c / q_ps_npatoshopc_c_y2y):
    1. PCT_GROWTH T=2   (6-month pct growth)
    2. PCT_GROWTH T=4   (1-year YoY pct growth)
    3. PCT_GROWTH T=20  (5-year pct growth)
    4. SURPRISE_GROWTH T=8
    5. ACCEL T=4
    6. ACCEL T=20
    7. ROBUST_GROWTH T=4
    8. ROBUST_GROWTH T=20

  REV dimension (total operating income, q_ps_toi_c / q_ps_toi_c_y2y):
    9.  PCT_GROWTH T=2
    10. PCT_GROWTH T=4
    11. PCT_GROWTH T=20
    12. SURPRISE_GROWTH T=8
    13. ACCEL T=4
    14. ACCEL T=20
    15. ROBUST_GROWTH T=4
    16. ROBUST_GROWTH T=20
...
Factor direction : positive (higher combined NP+REV growth is better)
Factor category  : growth - NP+REV composite multi-method
"""
...
FACTOR_NAME = "growth_NP_REV"
FACTOR_DIRECTION = 1  # positive

# NP fields
NI_FIELD = "q_ps_npatoshopc_c"
NI_Y2Y_FIELD = "q_ps_npatoshopc_c_y2y"

# REV fields
REV_FIELD = "q_ps_toi_c"
REV_Y2Y_FIELD = "q_ps_toi_c_y2y"

# Thresholds
MIN_ABS_DENOM_PCT = 1.0
MIN_HIST_PERIODS_SURPRISE = 4
ACCEL_WINSOR_LO = -5.0
ACCEL_WINSOR_HI = 5.0
MIN_ABS_DENOM_ACCEL = 0.01
WINSOR_LO_PCT = 1.0
WINSOR_HI_PCT = 99.0
MIN_VALID_SUBS = 4
...
def _compute_pct_growth_panel(
    ...
    """Compute PCT_GROWTH = (x_t - x_{t-T}) / |x_{t-T}| over T quarters."""
    if qdf.empty:
        return None

    qdf_idx = qdf.set_index(["symbol", "date"])
    val_lookup = qdf_idx[val_field]
    ...
    x_t = qdf[val_field].values.astype(float)
...
def _compute_surprise_growth_panel(
    ...
    """Compute SURPRISE_GROWTH = (x_t - mean(x_{t-1}..x_{t-T})) / std(...)."""
    if qdf.empty:
        return None
...
def _compute_accel_panel(
    ...
    """Compute ACCEL sub-factor for a y2y field with window T."""
    if qdf_y2y.empty:
        return None

    qdf_y2y = qdf_y2y.copy()
    qdf_y2y[y2y_field] = qdf_y2y[y2y_field].clip(ACCEL_WINSOR_LO, ACCEL_WINSOR_HI)
    ...
    for symbol, grp in qdf_y2y.groupby("symbol", sort=False):
        ...
        g_arr = grp_sorted[y2y_field].values.astype(float)
...
def _compute_robust_growth_panel(
    ...
    """Compute ROBUST_GROWTH sub-factor for val_field with window T."""
    if qdf.empty:
        return None
    ...
    for symbol, grp in qdf.groupby("symbol", sort=False):
        grp_sorted = grp.sort_values("date")
        idx = grp_sorted.index
        vals = grp_sorted[val_field].values.astype(float)
        rd_arr = grp_sorted["report_date"].values.astype("datetime64[ns]")
...
def _compute_sub_panels_for_field(
    raw: pd.DataFrame,
    val_field: str,
    y2y_field: str,
    prefix: str,
    ...
    """Compute 8 sub-factor panels for one dimension (NP or REV)."""
    qdf = _build_quarterly_panel(raw, val_field)
    qdf_y2y = _build_quarterly_panel(raw, y2y_field) if y2y_field in raw.columns else pd.DataFrame()

    panels = []
    names = []

    for T in [2, 4, 20]:
        panel = _compute_pct_growth_panel(qdf, val_field, T, trading_dates, end_date)
        if panel is not None and not panel.empty:
            panel.columns = trading_dates
            panels.append(panel)
            names.append(f"{prefix}_pct_T{T}")

    panel = _compute_surprise_growth_panel(qdf, val_field, 8, trading_dates, end_date)
    if panel is not None and not panel.empty:
        panels.append(panel)
        names.append(f"{prefix}_surprise_T8")

    if not qdf_y2y.empty:
        for T in [4, 20]:
            panel = _compute_accel_panel(qdf_y2y, y2y_field, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                panels.append(panel)
                names.append(f"{prefix}_accel_T{T}")
    else:
        warnings.warn(f"growth_NP_REV: field '{y2y_field}' not available; ACCEL skipped.")

    for T in [4, 20]:
        panel = _compute_robust_growth_panel(qdf, val_field, T, trading_dates, end_date)
        if panel is not None and not panel.empty:
            panels.append(panel)
            names.append(f"{prefix}_robust_T{T}")
...
class growth_NP_REV(FundamentalFactorCalculator):
    """
    Combined Net Profit + Revenue Growth Composite factor.

    Combines 16 sub-factors (8 for NP, 8 for REV):
      NP: PCT_GROWTH (T=2,4,20), SURPRISE_GROWTH (T=8),
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        Steps:
          ...
          2. Compute 8 NP sub-factor panels + 8 REV sub-factor panels.
          3. Per sub-factor: cross-section winsorize + z-score.
          4. Equal-weight average (need >= MIN_VALID_SUBS valid per cell).
          5. Cross-section rank normalize to [0, 1].
          6. Filter to A-share mainboard stocks.
        """
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("growth_NP_REV: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Step 2: Compute sub-panels for NP and REV dimensions
        ni_panels, ni_names = _compute_sub_panels_for_field(
            raw, NI_FIELD, NI_Y2Y_FIELD, "ni", trading_dates, end_date
        )
        rev_panels, rev_names = _compute_sub_panels_for_field(
            raw, REV_FIELD, REV_Y2Y_FIELD, "rev", trading_dates, end_date
        )

        sub_panels = ni_panels + rev_panels

        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"growth_NP_REV: only {len(sub_panels)} valid sub-panels "
                f"computed, need >= {MIN_VALID_SUBS}"
            )

        # Step 3: Winsorize + z-score each sub-panel
        normed_panels = []
        ...
        # Step 4: Align to union symbols and equal-weight average
        all_symbols = sorted(
        ...
        # Step 5: Cross-section rank normalization
        composite = _cross_section_rank_norm(composite)

        # Step 6: Mainboard filter
        ...
        if len(symbols_arr) == 0:
            raise ValueError("growth_NP_REV: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"growth_NP_REV NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python growth_np_rev.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("growth_NP_REV factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute growth_NP_REV factor")
    calculator = growth_NP_REV()
