"""
REV_LT_FT factor (Revenue Long-Term Forecast Trend)

Computes the revision momentum of analyst FY3 operating revenue consensus:
  REV_LT_FT_1Q = (FY3_consensus(t) - FY3_consensus(t-65))  / FY3_consensus(t-65)
  REV_LT_FT_6M = (FY3_consensus(t) - FY3_consensus(t-130)) / FY3_consensus(t-130)

  REV_LT_FT = 0.5 * rank(REV_LT_FT_1Q) + 0.5 * rank(REV_LT_FT_6M)
...
Data source:
  - tushare.report_rc : op_rt (10k CNY), quarter, report_date, org_name, ts_code
...
Factor direction: positive (upward revision of long-term revenue forecasts is better)
...
def _fy3_year(date: pd.Timestamp) -> int:
    """FY3 target year: always t.year + 2 (rolls over on January 1st)."""
    return date.year + 2
...
class REV_LT_FT(FundamentalFactorCalculator):
    """
    Revenue Long-Term Forecast Trend factor.

    Measures the momentum of analyst FY3 operating revenue consensus revisions
    over two windows (1Q = 65 trading days, 6M = 130 trading days).
    FY3 = t.year + 2 (the third fiscal year from the current date).
    Denominator is not abs(): negative/zero revenue denominator -> NaN.
    """
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        if N == 0:
            raise ValueError("REV_LT_FT: no mainboard symbols found in tushare data")

        # Step 2: Build FY3 consensus panel from tushare (10k CNY)
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"REV_LT_FT: NaN ratio is high ({nan_ratio:.1%}). "
                "Expected if few stocks have FY3 analyst coverage."
            )
    ...
    def _get_mainboard_symbols(self, fd: FundamentalData) -> np.ndarray:
        ...
        Using tushare directly gives a stable universe independent of end_date,
        avoiding false leakage signals from cross-section rank differences.
        """
        try:
            ...
            warnings.warn(f"REV_LT_FT: cannot load tushare symbols: {exc}")
    ...
    def _build_consensus_panel(
        ...
        """
        Build daily FY3 analyst consensus revenue panel from tushare.report_rc.

        For each trading date t:
          - FY3 year = t.year + 2
          - Look at reports with report_date in [t - 180 days, t]
            AND quarter == f"{t.year + 2}Q4"
          - Per org_name, take the most recent report
          - Require >= MIN_ANALYST_COUNT analysts; take median of op_rt (10k CNY)

        Returns:
            ndarray (N, T) float64, units = 10k CNY. NaN where insufficient coverage.
        """
        N = len(symbols)
        T = len(trading_dates)
        ...
        try:
            ...
            warnings.warn(f"REV_LT_FT: failed to load tushare data: {exc}")
            return result
        ...
        for t_idx, t in enumerate(dates_d):
            ...
            fy3_window = window[window['q_year'] == fy3_year]
            if fy3_window.empty:
                continue
...
# ------------------------------------------------------------------
# Smoke test (run: python rev_lt_ft.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("REV_LT_FT factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute REV_LT_FT factor")
    calculator = REV_LT_FT()
    ...
    # FY3 factors have inherently higher NaN than FY1 factors:
    # (1) fewer analysts publish 3-year-ahead forecasts,
    # (2) the FY3 target year resets on Jan 1, causing NaN for windows spanning year-end.
    # Allow up to 95% NaN for this factor (vs 80% for typical factors).
