"""
RD_TO_REV factor (R&D Expense to Revenue Ratio, TTM)

Formula: RD_TTM / Revenue_TTM
...
Factor direction: positive (higher R&D intensity predicts better long-term performance)
...
Boundary conditions:
  ...
  - RD_TTM > Revenue_TTM      -> kept (extreme but valid; downstream Winsorize handles it)
"""
...
FACTOR_NAME = "RD_TO_REV"
FACTOR_DIRECTION = 1  # positive: higher R&D intensity is better


class RD_TO_REV(FundamentalFactorCalculator):
    """
    R&D Expense to Revenue Ratio (TTM).

    RD_TO_REV = RD_TTM / Revenue_TTM
    where:
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Compute RD_TO_REV daily panel (N stocks x T days).

        Returns:
            FactorData with values = RD_TTM / Revenue_TTM (dimensionless ratio)
        """
        # Ensure raw data is loaded; retrieve trading calendar via a valuation field
        # (we don't need mc for the ratio itself, but need trading dates)
        ...
        rd_v, rd_s, rd_d = self._compute_ttm_panel(
        ...
        if rd_v.size == 0:
            raise ValueError("RD_TO_REV: q_ps_rade_c TTM panel is empty")
        if rev_v.size == 0:
            raise ValueError("RD_TO_REV: q_ps_toi_c TTM panel is empty")

        # --- 2. Common symbols ---
        common_syms = np.intersect1d(rd_s, rev_s)
        if len(common_syms) == 0:
            raise ValueError("RD_TO_REV: no common symbols between RD and Revenue panels")

        N = len(common_syms)
        T = len(trading_dates_arr)

        rd_si  = np.searchsorted(rd_s,  common_syms)
        rev_si = np.searchsorted(rev_s, common_syms)

        rd_aligned  = rd_v[rd_si]   # (N, T)
        ...
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(
                f"RD_TO_REV NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python rd_to_rev.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("RD_TO_REV factor smoke test")
    print("=" * 60)
