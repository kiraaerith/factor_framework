"""
ROA_GROWTH_COMP factor (ROA Composite Growth)

Synthesizes 8 sub-factor components across multiple time windows and growth
metrics to capture multi-dimensional ROA growth dynamics.

Components:
  1. PCT_GROWTH  T=2   (semi-annual ROA change)
  2. PCT_GROWTH  T=4   (annual ROA change)
  3. PCT_GROWTH  T=20  (5-year ROA change)
  4. SURPRISE_GROWTH T=8 (ROA vs 2-year history)
...
Data source:
  lixinger.financial_statements.q_m_roa_t  (TTM ROA, quarterly)

Factor direction: positive (higher composite ROA growth is better)
Factor category: growth - profitability quality dimension

Notes:
  - ROA is a ratio; absolute difference (pp change) is used in ACCEL.
  - Pre-winsorize raw ROA at +-50% before all sub-factor computation.
...
FACTOR_NAME = "ROA_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive: higher ROA growth is better

# Tuning constants
ROA_WINSORIZE_CLIP = 0.5    # pre-winsorize raw ROA at +-50%
ACCEL_DELTA_CLIP   = 0.2    # ACCEL delta winsorize at +-20pp
ACCEL_MIN_DENOM    = 0.002  # ACCEL min |delta_{t-T}| to avoid explosion (0.2pp)
PCT_MIN_ABS_DENOM  = 0.001  # PCT_GROWTH min |ROA_{t-T}| (0.1%)
...
class ROA_GROWTH_COMP(FundamentalFactorCalculator):
    """
    ROA Composite Growth factor.

    Compute 8 sub-factors in the quarterly domain (using `q_m_roa_t` lags),
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Compute ROA_GROWTH_COMP daily panel (N stocks x T days).

        Steps:
          1. Extract q_m_roa_t quarterly rows from raw data.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("ROA_GROWTH_COMP: no raw fundamental data available")

        # --- Step 1: Extract quarterly data ---
        raw = fundamental_data._raw_data[
            ["symbol", "date", "report_date", "q_m_roa_t"]
        ].copy()

        raw = raw.dropna(subset=["q_m_roa_t"])
        ...
        if raw.empty:
            raise ValueError("ROA_GROWTH_COMP: no q_m_roa_t data after filtering")
        ...
        raw["roa"] = raw["q_m_roa_t"].clip(-ROA_WINSORIZE_CLIP, ROA_WINSORIZE_CLIP)

        # Build (symbol, fiscal_date) → (roa, report_date) lookup.
        ...
        for lag in range(1, 25):
            ...
            lag_roa = roa_lookup.reindex(midx).values.astype(float)
            ...
            raw[f"lag{lag}"] = np.where(pit_ok, lag_roa, np.nan)

        # Compute rolling stats from PIT-filtered lag arrays (avoids positional
        # window leakage through missing quarters).
        ...
        # robust4_std: roa + lag1..lag4 (5 quarters, min 3)
        r4_arr = raw[["roa"] + [f"lag{i}" for i in range(1, 5)]].values
        ...
        # robust20_std: roa + lag1..lag20 (21 quarters, min 11)
        r20_arr = raw[["roa"] + [f"lag{i}" for i in range(1, 21)]].values
        ...
        raw["comp1"] = _pct_growth(raw["roa"], raw["lag2"])

        # comp2: PCT_GROWTH T=4
        raw["comp2"] = _pct_growth(raw["roa"], raw["lag4"])

        # comp3: PCT_GROWTH T=20
        raw["comp3"] = _pct_growth(raw["roa"], raw["lag20"])

        # comp4: SURPRISE_GROWTH T=8
        raw["comp4"] = _surprise_growth(raw["roa"], raw["surp_mean"], raw["surp_std"])

        # comp5: ACCEL T=4
        #   delta_t   = roa - lag4
        #   delta_{t-4} = lag4 - lag8
        raw["comp5"] = _accel(raw["roa"], raw["lag4"], raw["lag4"], raw["lag8"])

        # comp6: ACCEL T=20
        #   delta_t    = roa - lag4
        #   delta_{t-20} = lag20 - lag24
        raw["comp6"] = _accel(raw["roa"], raw["lag4"], raw["lag20"], raw["lag24"])

        # comp7: ROBUST_GROWTH T=4
        raw["comp7"] = _robust_growth(raw["roa"], raw["lag4"], raw["robust4_std"])

        # comp8: ROBUST_GROWTH T=20
        raw["comp8"] = _robust_growth(raw["roa"], raw["lag20"], raw["robust20_std"])
        ...
        if not normalized:
            raise ValueError(
                "ROA_GROWTH_COMP: all sub-factor panels are empty"
            )
...
def _accel(roa_t: pd.Series,
           roa_lag4: pd.Series,
           roa_lag_T: pd.Series,
           roa_lag_T4: pd.Series) -> pd.Series:
    """
    ACCEL = (delta_t - delta_{t-T}) / |delta_{t-T}|
      where delta_t     = roa_t - roa_{t-4}   (current YoY absolute change)
            delta_{t-T} = roa_{t-T} - roa_{t-T-4}

    Winsorize deltas at +-ACCEL_DELTA_CLIP before division.
    NaN when: |delta_{t-T}| < ACCEL_MIN_DENOM, or any node is NaN.
    """
    delta_t  = (roa_t    - roa_lag4 ).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    delta_tm = (roa_lag_T - roa_lag_T4).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    ...
    all_valid = (
        ~roa_t.isna() & ~roa_lag4.isna() &
        ~roa_lag_T.isna() & ~roa_lag_T4.isna()
    )
...
# ------------------------------------------------------------------
# Smoke test (run: python roa_growth_comp.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("ROA_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute ROA_GROWTH_COMP factor")
    calculator = ROA_GROWTH_COMP()
