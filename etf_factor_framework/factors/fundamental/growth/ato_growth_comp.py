"""
ATO_GROWTH_COMP factor (Total Asset Turnover Composite Growth)

Synthesizes 8 sub-factor components across multiple time windows and growth
metrics to capture multi-dimensional ATO growth dynamics.

Components:
  1. PCT_GROWTH  T=2   (semi-annual ATO change)
  2. PCT_GROWTH  T=4   (annual ATO change)
  3. PCT_GROWTH  T=20  (5-year ATO change)
  4. SURPRISE_GROWTH T=8 (ATO vs 2-year history)
...
Data source:
  lixinger.financial_statements.q_m_ta_to_t  (total asset turnover, quarterly)

Factor direction: positive (higher composite ATO growth is better)
Factor category: growth - operational efficiency dimension

Notes:
  - ATO is a ratio (non-negative for normal companies); absolute difference is
    used in ACCEL (not percentage change), consistent with ROA_GROWTH_COMP.
  - Pre-winsorize raw ATO at [0, 5] before all sub-factor computation.
...
FACTOR_NAME = "ATO_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive: higher ATO growth is better

# Tuning constants
ATO_WINSORIZE_LO   = 0.0    # pre-winsorize lower bound (ATO >= 0)
ATO_WINSORIZE_HI   = 5.0    # pre-winsorize upper bound (5x turnover is extreme)
ACCEL_DELTA_CLIP   = 0.5    # ACCEL delta winsorize at +-0.5 (times/year)
ACCEL_MIN_DENOM    = 0.005  # ACCEL min |delta_{t-T}| to avoid explosion
PCT_MIN_ABS_DENOM  = 0.01   # PCT_GROWTH min |ATO_{t-T}| (0.01 times/year)
...
class ATO_GROWTH_COMP(FundamentalFactorCalculator):
    """
    Total Asset Turnover Composite Growth factor.

    Compute 8 sub-factors in the quarterly domain (using `q_m_ta_to_t` lags),
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Steps:
          1. Extract q_m_ta_to_t quarterly rows from raw data.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("ATO_GROWTH_COMP: no raw fundamental data available")

        # --- Step 1: Extract quarterly data ---
        raw = fundamental_data._raw_data[
            ["symbol", "date", "report_date", "q_m_ta_to_t"]
        ].copy()

        raw = raw.dropna(subset=["q_m_ta_to_t"])
        raw = raw[raw["report_date"] <= fundamental_data.end_date].copy()

        # Sanity filter: fiscal date must not be after report_date
        raw = raw[raw["date"] <= raw["report_date"]].copy()

        if raw.empty:
            raise ValueError("ATO_GROWTH_COMP: no q_m_ta_to_t data after filtering")
        ...
        raw["ato"] = raw["q_m_ta_to_t"].clip(ATO_WINSORIZE_LO, ATO_WINSORIZE_HI)

        # Build (symbol, fiscal_date) -> (ato, report_date) lookup.
        # PIT constraint: lag is valid only if lag quarter's report_date
        # <= current row's report_date.
        raw_idx = raw.set_index(["symbol", "date"])
        ato_lookup = raw_idx["ato"]
        rd_lookup  = raw_idx["report_date"]

        # Compute lags 1-24 (fiscal-date-based) with PIT filter
        for lag in range(1, 25):
            lag_dates = raw["date"] - pd.DateOffset(months=3 * lag)
            midx = pd.MultiIndex.from_arrays([raw["symbol"], lag_dates])
            lag_ato = ato_lookup.reindex(midx).values.astype(float)
            ...
            raw[f"lag{lag}"] = np.where(pit_ok, lag_ato, np.nan)
        ...
        # robust4_std: ato + lag1..lag4 (5 quarters, min 3)
        r4_arr = raw[["ato"] + [f"lag{i}" for i in range(1, 5)]].values
        ...
        # robust20_std: ato + lag1..lag20 (21 quarters, min 11)
        r20_arr = raw[["ato"] + [f"lag{i}" for i in range(1, 21)]].values
        ...
        raw["comp1"] = _pct_growth(raw["ato"], raw["lag2"])

        # comp2: PCT_GROWTH T=4
        raw["comp2"] = _pct_growth(raw["ato"], raw["lag4"])

        # comp3: PCT_GROWTH T=20
        raw["comp3"] = _pct_growth(raw["ato"], raw["lag20"])

        # comp4: SURPRISE_GROWTH T=8
        raw["comp4"] = _surprise_growth(raw["ato"], raw["surp_mean"], raw["surp_std"])

        # comp5: ACCEL T=4
        #   delta_t     = ato - lag4
        #   delta_{t-4} = lag4 - lag8
        raw["comp5"] = _accel(raw["ato"], raw["lag4"], raw["lag4"], raw["lag8"])

        # comp6: ACCEL T=20
        #   delta_t      = ato - lag4
        #   delta_{t-20} = lag20 - lag24
        raw["comp6"] = _accel(raw["ato"], raw["lag4"], raw["lag20"], raw["lag24"])

        # comp7: ROBUST_GROWTH T=4
        raw["comp7"] = _robust_growth(raw["ato"], raw["lag4"], raw["robust4_std"])

        # comp8: ROBUST_GROWTH T=20
        raw["comp8"] = _robust_growth(raw["ato"], raw["lag20"], raw["robust20_std"])
        ...
        if not normalized:
            raise ValueError(
                "ATO_GROWTH_COMP: all sub-factor panels are empty"
            )
...
def _accel(ato_t: pd.Series,
           ato_lag4: pd.Series,
           ato_lag_T: pd.Series,
           ato_lag_T4: pd.Series) -> pd.Series:
    """
    ACCEL = (delta_t - delta_{t-T}) / |delta_{t-T}|
      where delta_t     = ato_t - ato_{t-4}   (current YoY absolute change)
            delta_{t-T} = ato_{t-T} - ato_{t-T-4}

    Winsorize deltas at +-ACCEL_DELTA_CLIP before division.
    NaN when: |delta_{t-T}| < ACCEL_MIN_DENOM, or any node is NaN.
    """
    delta_t  = (ato_t     - ato_lag4 ).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    delta_tm = (ato_lag_T - ato_lag_T4).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    ...
    all_valid = (
        ~ato_t.isna() & ~ato_lag4.isna() &
        ~ato_lag_T.isna() & ~ato_lag_T4.isna()
    )
...
# ------------------------------------------------------------------
# Smoke test (run: python ato_growth_comp.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("ATO_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute ATO_GROWTH_COMP factor")
    calculator = ATO_GROWTH_COMP()
