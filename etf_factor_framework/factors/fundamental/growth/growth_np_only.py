"""
growth_NP_only factor (Net Profit Growth Composite)

Simplified version of growth_NP_REV: NP dimension only (8 sub-factors),
REV dimension completely removed.

Sub-factors (NP dimension, q_ps_npatoshopc_c / q_ps_npatoshopc_c_y2y):
...
Rationale: Revenue growth is a noisier signal than net profit growth.
  High revenue growth with margin compression can be a value trap.
  Using only the NP dimension yields a purer profitability signal.

Data source : lixinger.financial_statements
Factor direction : positive (higher net profit growth is better)
Factor category  : growth - NP composite multi-method
"""
...
class growth_NP_only(FundamentalFactorCalculator):
    """
    Net Profit Growth Composite factor (NP dimension only).

    8 sub-factors based on attributable net profit (q_ps_npatoshopc_c):
      PCT_GROWTH (T=2, 4, 20), SURPRISE_GROWTH (T=8),
      ACCEL (T=4, 20), ROBUST_GROWTH (T=4, 20)

    Simplified version of growth_NP_REV with REV dimension removed.
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        Steps:
          ...
          2. Compute 8 NP sub-factor panels.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("growth_NP_only: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Build quarterly panels
        qdf = _build_quarterly_panel(raw, NI_FIELD)
        qdf_y2y = _build_quarterly_panel(raw, NI_Y2Y_FIELD) if NI_Y2Y_FIELD in raw.columns else pd.DataFrame()

        sub_panels = []

        # PCT_GROWTH T=2, 4, 20
        for T in [2, 4, 20]:
            panel = _compute_pct_growth_panel(qdf, NI_FIELD, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                panel.columns = trading_dates
                sub_panels.append(panel)

        # SURPRISE_GROWTH T=8
        panel = _compute_surprise_growth_panel(qdf, NI_FIELD, 8, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
        ...
        if not qdf_y2y.empty:
            for T in [4, 20]:
                panel = _compute_accel_panel(qdf_y2y, NI_Y2Y_FIELD, T, trading_dates, end_date)
                if panel is not None and not panel.empty:
                    sub_panels.append(panel)
        else:
            warnings.warn(f"growth_NP_only: field '{NI_Y2Y_FIELD}' not available; ACCEL skipped.")

        # ROBUST_GROWTH T=4, 20
        for T in [4, 20]:
            panel = _compute_robust_growth_panel(qdf, NI_FIELD, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)

        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"growth_NP_only: only {len(sub_panels)} valid sub-panels "
                f"computed, need >= {MIN_VALID_SUBS}"
            )

        # Winsorize + z-score each sub-panel
        normed_panels = []
        ...
        # Align to union symbols and equal-weight average
        all_symbols = sorted(
        ...
        # Cross-section rank normalization
        ...
        if len(symbols_arr) == 0:
            raise ValueError("growth_NP_only: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"growth_NP_only NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python growth_np_only.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("growth_NP_only factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute growth_NP_only factor")
    calculator = growth_NP_only()
