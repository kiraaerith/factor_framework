"""
RD_TO_NP factor (R&D Expense to Net Profit Ratio, TTM)

Formula: RD_TTM / NetProfit_TTM
...
Factor direction: positive (higher R&D reinvestment intensity relative to profit predicts
...
Boundary conditions:
  ...
  - RD_to_NP > 5 (extreme outlier)  -> kept (downstream Winsorize handles it)
"""
...
FACTOR_NAME = "RD_TO_NP"
FACTOR_DIRECTION = 1  # positive: higher R&D reinvestment rate is better


class RD_TO_NP(FundamentalFactorCalculator):
    """
    R&D Expense to Net Profit Ratio (TTM).

    RD_TO_NP = RD_TTM / NetProfit_TTM
    where:
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Compute RD_TO_NP daily panel (N stocks x T days).

        Returns:
            FactorData with values = RD_TTM / NetProfit_TTM (dimensionless ratio)
        """
        ...
        if rd_v.size == 0:
            raise ValueError("RD_TO_NP: q_ps_rade_c TTM panel is empty")
        if np_v.size == 0:
            raise ValueError("RD_TO_NP: q_ps_npatoshopc_c TTM panel is empty")

        # --- 2. Common symbols ---
        common_syms = np.intersect1d(rd_s, np_s)
        if len(common_syms) == 0:
            raise ValueError("RD_TO_NP: no common symbols between RD and NetProfit panels")
        ...
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(
                f"RD_TO_NP NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python rd_to_np.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("RD_TO_NP factor smoke test")
    print("=" * 60)
