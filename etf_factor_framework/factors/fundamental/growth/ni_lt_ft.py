"""
NI_LT_FT factor (Net Income Long-Term Forecast Trend)

Computes the revision momentum of analyst FY3 net profit consensus:
  NI_LT_FT_1Q = (FY3_consensus(t) - FY3_consensus(t-65))  / |FY3_consensus(t-65)|
  NI_LT_FT_6M = (FY3_consensus(t) - FY3_consensus(t-130)) / |FY3_consensus(t-130)|

  NI_LT_FT = 0.5 * rank(NI_LT_FT_1Q) + 0.5 * rank(NI_LT_FT_6M)
...
Post-processing per window:
  1. Winsorize to [-1, 5]
  2. Additional ±3σ winsorization per cross-section
...
Factor direction: positive (upward revision of long-term earnings forecasts is better)
...
def _winsorize_panel(panel: np.ndarray, lower: float, upper: float) -> np.ndarray:
    """
    Winsorize each cross-section to [lower, upper], then apply ±3σ per cross-section.
...
class NI_LT_FT(FundamentalFactorCalculator):
    """
    Net Income Long-Term Forecast Trend factor.

    Measures the momentum of analyst FY3 net profit consensus revisions
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        if N == 0:
            raise ValueError("NI_LT_FT: no mainboard symbols found in tushare data")

        # Step 2: Build FY3 consensus panel from tushare (万元)
        ...
        # Step 5: Winsorize each window: [-1, 5] then ±3σ per cross-section
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"NI_LT_FT: NaN ratio is high ({nan_ratio:.1%}). "
    ...
    def _get_mainboard_symbols(self, fd: FundamentalData) -> np.ndarray:
        ...
        try:
            ...
            warnings.warn(f"NI_LT_FT: cannot load tushare symbols: {exc}")
    ...
    def _build_consensus_panel(
        ...
        """
        Build daily FY3 analyst consensus NI panel from tushare.report_rc.
        ...
        try:
            ...
            warnings.warn(f"NI_LT_FT: failed to load tushare data: {exc}")
            return result
        ...
        for t_idx, t in enumerate(dates_d):
            ...
            # fc_df is already sorted by (report_date_dt, ts_code, org_name, np_val)
...
# ------------------------------------------------------------------
# Smoke test (run: python ni_lt_ft.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("NI_LT_FT factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute NI_LT_FT factor")
    calculator = NI_LT_FT()
