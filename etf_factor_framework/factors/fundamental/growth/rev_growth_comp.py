"""
REV_GROWTH_COMP factor (Total Operating Revenue Growth Composite)

Computes a composite of 8 sub-factors, all based on total operating income
(q_ps_toi_c) and its YoY growth rate (q_ps_toi_c_y2y):
...
Factor direction : positive (higher composite revenue growth is better)
Factor category  : growth - revenue composite multi-method

Notes:
  - PCT_GROWTH denominator = |x_{t-T}|; near-zero (<1 yuan) -> NaN.
  - SURPRISE_GROWTH requires >= 4 valid historical quarters; std=0 -> NaN.
  - ACCEL uses q_ps_toi_c_y2y winsorized to [-5, 5];
...
FACTOR_NAME = "REV_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive

# Fields
REV_FIELD = "q_ps_toi_c"          # single-quarter total operating income
REV_Y2Y_FIELD = "q_ps_toi_c_y2y"  # pre-computed YoY growth rate
...
def _compute_pct_growth_panel(
    ...
    """
    Compute PCT_GROWTH = (x_t - x_{t-T}) / |x_{t-T}| for REV_FIELD over T quarters.
    ...
    x_t = qdf[REV_FIELD].values.astype(float)
...
def _compute_accel_panel(
    ...
    """
    Compute ACCEL sub-factor for q_ps_toi_c_y2y with window T.
    """
    if qdf_y2y.empty:
        return None
    ...
    qdf_y2y[REV_Y2Y_FIELD] = qdf_y2y[REV_Y2Y_FIELD].clip(
    ...
    for symbol, grp in qdf_y2y.groupby("symbol", sort=False):
        ...
        g_arr = grp_sorted[REV_Y2Y_FIELD].values.astype(float)
...
def _compute_robust_growth_panel(
    ...
    """Compute ROBUST_GROWTH sub-factor for REV_FIELD with window T."""
    if qdf.empty:
        return None
    ...
    for symbol, grp in qdf.groupby("symbol", sort=False):
        grp_sorted = grp.sort_values("date")
        idx = grp_sorted.index
        vals = grp_sorted[REV_FIELD].values.astype(float)
        rd_arr = grp_sorted["report_date"].values.astype("datetime64[ns]")
...
class REV_GROWTH_COMP(FundamentalFactorCalculator):
    """
    Total Operating Revenue Growth Composite factor.

    Combines 8 sub-factors for total operating revenue growth:
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Compute REV_GROWTH_COMP daily panel (N_stocks x T_days).

        Steps:
          1. Load raw fundamental data.
          2. Build deduplicated quarterly panels for REV_FIELD and REV_Y2Y_FIELD.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("REV_GROWTH_COMP: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Step 2: Build deduplicated quarterly panels
        qdf_rev = _build_quarterly_panel(raw, REV_FIELD)
        qdf_y2y = (
            _build_quarterly_panel(raw, REV_Y2Y_FIELD)
            if REV_Y2Y_FIELD in raw.columns
            else pd.DataFrame()
        )
        ...
        # PCT_GROWTH: T=2, 4, 20
        for T in [2, 4, 20]:
            panel = _compute_pct_growth_panel(qdf_rev, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
                sub_names.append(f"pct_T{T}")

        # SURPRISE_GROWTH: T=8
        panel = _compute_surprise_growth_panel(qdf_rev, 8, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
            sub_names.append("surprise_T8")

        # ACCEL: T=4, 20 (uses REV_Y2Y_FIELD)
        if not qdf_y2y.empty:
            ...
            warnings.warn(
                f"REV_GROWTH_COMP: field '{REV_Y2Y_FIELD}' not available; "
                "ACCEL sub-factors skipped."
            )

        # ROBUST_GROWTH: T=4, 20
        for T in [4, 20]:
            panel = _compute_robust_growth_panel(qdf_rev, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
        ...
        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"REV_GROWTH_COMP: only {len(sub_panels)} valid sub-panels "
        ...
        if len(symbols_arr) == 0:
            raise ValueError("REV_GROWTH_COMP: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"REV_GROWTH_COMP NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python rev_growth_comp.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("REV_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute REV_GROWTH_COMP factor")
    calculator = REV_GROWTH_COMP()
