"""
ROIC_GROWTH_COMP factor (ROIC Composite Growth)

Synthesizes 8 sub-factor components across multiple time windows and growth
metrics to capture multi-dimensional ROIC growth dynamics.

Components:
  1. PCT_GROWTH  T=2   (semi-annual ROIC change)
  2. PCT_GROWTH  T=4   (annual ROIC change)
  3. PCT_GROWTH  T=20  (5-year ROIC change)
  4. SURPRISE_GROWTH T=8 (ROIC vs 2-year history)
  5. ACCEL T=4         (1-year YoY acceleration)
  6. ACCEL T=20        (5-year YoY acceleration)
  7. ROBUST_GROWTH T=4  (1-year risk-adjusted growth)
  8. ROBUST_GROWTH T=20 (5-year risk-adjusted growth)

Data source:
  lixinger.financial_statements.q_m_roic_t  (single-quarter ROIC, period value)

Factor direction: positive (higher composite ROIC growth is better)
Factor category: growth - profitability quality dimension

Notes:
  - ROIC is a ratio; absolute difference (pp change) is used in ACCEL.
  - Pre-winsorize raw ROIC at +-200% before all sub-factor computation.
  - ACCEL delta is winsorized at +-50pp (0.5) before division.
  ...
  - Financial stocks (banks, insurance) commonly lack q_m_roic_t data;
    they will have NaN factor values and are partially handled by
    the mainboard filter and industry neutralization.
"""
...
FACTOR_NAME = "ROIC_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive: higher ROIC growth is better

# Tuning constants
ROIC_WINSORIZE_CLIP = 2.0    # pre-winsorize raw ROIC at +-200%
ACCEL_DELTA_CLIP   = 0.5    # ACCEL delta winsorize at +-50pp (spec)
ACCEL_MIN_DENOM    = 0.005  # ACCEL min |delta_{t-T}| (0.5pp, spec)
PCT_MIN_ABS_DENOM  = 0.001  # PCT_GROWTH min |ROIC_{t-T}| (0.1%)
...
class ROIC_GROWTH_COMP(FundamentalFactorCalculator):
    """
    ROIC Composite Growth factor.

    Compute 8 sub-factors in the quarterly domain (using `q_m_roic_t` lags),
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Steps:
          1. Extract q_m_roic_t quarterly rows from raw data.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("ROIC_GROWTH_COMP: no raw fundamental data available")

        # --- Step 1: Extract quarterly data ---
        raw = fundamental_data._raw_data[
            ["symbol", "date", "report_date", "q_m_roic_t"]
        ].copy()

        raw = raw.dropna(subset=["q_m_roic_t"])
        ...
        # errors; they appear in the full run (loaded via SQL date <= 2025)
        # but not in the truncated run (SQL date <= cutoff), causing
        # cross-sectional universe divergence at overlap dates.
        raw = raw[raw["date"] <= raw["report_date"]].copy()

        if raw.empty:
            raise ValueError("ROIC_GROWTH_COMP: no q_m_roic_t data after filtering")

        # Deduplicate by (symbol, fiscal date): keep the EARLIEST published
        # version of each fiscal quarter (PIT-safe). Restated reports
        # published after the end_date cutoff would be absent in truncated
        # runs, so using the first-published version ensures lag values are
        # identical between full and truncated runs (leakage-free).
        ...
        raw["roic"] = raw["q_m_roic_t"].clip(-ROIC_WINSORIZE_CLIP, ROIC_WINSORIZE_CLIP)

        # Build (symbol, fiscal_date) → (roic, report_date) lookup.
        # PIT constraint: a lag is valid only if the lag quarter's report_date
        # ≤ the current row's report_date. This prevents a delayed/restated
        # fiscal quarter (fiscal_date < cutoff, report_date > cutoff) from
        # contributing a lag value in the full run that is absent in the
        # truncated run — causing universe-level z-score divergence.
        raw_idx = raw.set_index(["symbol", "date"])
        roic_lookup = raw_idx["roic"]
        ...
        for lag in range(1, 25):
            ...
            lag_roic = roic_lookup.reindex(midx).values.astype(float)
            lag_rd   = rd_lookup.reindex(midx).values   # dtype object/datetime
            # PIT: NaN out lags whose report_date > current row's report_date
            ...
            raw[f"lag{lag}"] = np.where(pit_ok, lag_roic, np.nan)
        ...
        # robust4_std: roic + lag1..lag4 (5 quarters, min 3)
        r4_arr = raw[["roic"] + [f"lag{i}" for i in range(1, 5)]].values
        ...
        # robust20_std: roic + lag1..lag20 (21 quarters, min 11)
        r20_arr = raw[["roic"] + [f"lag{i}" for i in range(1, 21)]].values
        ...
        raw["comp1"] = _pct_growth(raw["roic"], raw["lag2"])

        # comp2: PCT_GROWTH T=4
        raw["comp2"] = _pct_growth(raw["roic"], raw["lag4"])

        # comp3: PCT_GROWTH T=20
        raw["comp3"] = _pct_growth(raw["roic"], raw["lag20"])

        # comp4: SURPRISE_GROWTH T=8
        raw["comp4"] = _surprise_growth(raw["roic"], raw["surp_mean"], raw["surp_std"])

        # comp5: ACCEL T=4
        #   delta_t     = roic - lag4
        #   delta_{t-4} = lag4 - lag8
        raw["comp5"] = _accel(raw["roic"], raw["lag4"], raw["lag4"], raw["lag8"])

        # comp6: ACCEL T=20
        #   delta_t      = roic - lag4
        #   delta_{t-20} = lag20 - lag24
        raw["comp6"] = _accel(raw["roic"], raw["lag4"], raw["lag20"], raw["lag24"])

        # comp7: ROBUST_GROWTH T=4
        raw["comp7"] = _robust_growth(raw["roic"], raw["lag4"], raw["robust4_std"])

        # comp8: ROBUST_GROWTH T=20
        raw["comp8"] = _robust_growth(raw["roic"], raw["lag20"], raw["robust20_std"])
        ...
        for col in comp_cols:
            ...
            syms = np.array(panel_df.index.tolist())

            pre_fill_nan = np.isnan(arr)
            arr = _industry_fill(arr, syms, industry_map)
            arr[pre_fill_nan] = np.nan  # don't fill "no data yet" positions (would cause leakage)
        ...
        if not normalized:
            raise ValueError(
                "ROIC_GROWTH_COMP: all sub-factor panels are empty"
            )
...
def _accel(roic_t: pd.Series,
           roic_lag4: pd.Series,
           roic_lag_T: pd.Series,
           roic_lag_T4: pd.Series) -> pd.Series:
    """
    ACCEL = (delta_t - delta_{t-T}) / |delta_{t-T}|
      where delta_t     = roic_t - roic_{t-4}   (current YoY absolute change)
            delta_{t-T} = roic_{t-T} - roic_{t-T-4}

    Winsorize deltas at +-ACCEL_DELTA_CLIP before division.
    NaN when: |delta_{t-T}| < ACCEL_MIN_DENOM, or any node is NaN.
    """
    delta_t  = (roic_t    - roic_lag4 ).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    delta_tm = (roic_lag_T - roic_lag_T4).clip(-ACCEL_DELTA_CLIP, ACCEL_DELTA_CLIP)
    ...
    all_valid = (
        ~roic_t.isna() & ~roic_lag4.isna() &
        ~roic_lag_T.isna() & ~roic_lag_T4.isna()
    )
...
def _industry_fill(arr: np.ndarray,
    ...
    for ind in unique_inds:
        mask = (industries == ind)          # (N,)
        group = arr[mask, :]                # (n_ind, T)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", RuntimeWarning)
            med = np.nanmedian(group, axis=0)  # (T,)
        nan_in_group = np.isnan(group)      # (n_ind, T)
        filled = np.where(nan_in_group, med, group)
        # Do not fill if median itself is NaN (whole industry missing)
...
def _winsorize_3sigma(arr: np.ndarray) -> np.ndarray:
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", RuntimeWarning)
        mean = np.nanmean(arr, axis=0, keepdims=True)   # (1, T)
        std  = np.nanstd(arr,  axis=0, ddof=1, keepdims=True)  # (1, T)
    lo = mean - 3.0 * std
    hi = mean + 3.0 * std
...
# ------------------------------------------------------------------
# Smoke test (run: python roic_growth_comp.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("ROIC_GROWTH_COMP factor smoke test")
    print("=" * 60)

    TEST_START = "2020-01-01"
    TEST_END   = "2024-12-31"
    # Use non-financial stocks to get meaningful ROIC data
    # (financial stocks like 601318/600036 have NaN ROIC by design)
    ...
    print(f"\n[Step 2] Compute ROIC_GROWTH_COMP factor")
    calculator = ROIC_GROWTH_COMP()
