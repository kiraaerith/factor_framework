"""
REV_FG factor (Revenue Forecast Growth Rate)

Computes:
  REV_FG = (ForecastRev_FY1 - ActualRev_t) / ActualRev_t

Data sources:
  - tushare.report_rc : op_rt (10k CNY = 万元), quarter (e.g. '2025Q4')
...
Unit alignment:
  ...
  - lixinger q_ps_oi_c is in 元

Denominator: ActualRev_t is used directly (no absolute value).
  - ActualRev_t <= 0 -> NaN (revenue should be positive; negative or zero is abnormal)

Winsorization: factor values clipped to [-1, 5] before returning.
Minimum analyst count: require >= 2 analysts in 180-day window (else NaN).

Factor direction: positive (higher expected revenue growth is better)
...
FACTOR_NAME = "REV_FG"
FACTOR_DIRECTION = 1  # positive: higher forecast revenue growth is better
...
class REV_FG(FundamentalFactorCalculator):
    """
    Revenue Forecast Growth Rate.

    REV_FG = (ForecastRev_FY1 - ActualRev_t) / ActualRev_t

    ForecastRev_FY1: median analyst consensus revenue for FY1 (tushare, 万元 -> 元)
    ActualRev_t: sum of single-quarter revenue over 4 quarters for most recent
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        if fd._raw_data is None or fd._raw_data.empty:
            raise ValueError("REV_FG: lixinger raw data is empty")

        T = len(trading_dates)
        dates_arr = np.array(trading_dates, dtype='datetime64[ns]')

        # Step 1: Build actual annual revenue panel from lixinger (元)
        actual_rev_panel, lx_symbols = self._load_actual_rev_panel(fd, trading_dates)

        # Step 2: Filter to mainboard
        mb_mask = np.array([_is_mainboard(s) for s in lx_symbols])
        lx_symbols = lx_symbols[mb_mask]
        actual_rev_panel = actual_rev_panel[mb_mask]

        if len(lx_symbols) == 0:
            raise ValueError("REV_FG: no mainboard symbols after filtering")

        # Step 3: Load FY1 analyst consensus panel from tushare (元)
        forecast_rev_panel = self._load_forecast_rev_panel(fd, lx_symbols, trading_dates)

        # Step 4: Compute growth rate (no absolute value in denominator)
        with np.errstate(divide='ignore', invalid='ignore'):
            growth = (forecast_rev_panel - actual_rev_panel) / actual_rev_panel

        # Mask invalid cases: ActualRev <= 0, either is NaN, or infinite
        bad = (
            (actual_rev_panel <= 0)
            | np.isnan(actual_rev_panel)
            | np.isnan(forecast_rev_panel)
            | np.isinf(growth)
        )
        growth[bad] = np.nan

        # Step 5: Winsorize to [-1, 5]
        finite_mask = ~np.isnan(growth)
        growth[finite_mask] = np.clip(growth[finite_mask], -1.0, 5.0)

        nan_ratio = np.isnan(growth).mean()
        if nan_ratio > 0.8:
            warnings.warn(
                f"REV_FG: NaN ratio is high ({nan_ratio:.1%}). "
    ...
    # ------------------------------------------------------------------
    # Internal: actual annual revenue from lixinger
    # ------------------------------------------------------------------

    def _load_actual_rev_panel(
        self,
        fd: FundamentalData,
        trading_dates: pd.DatetimeIndex,
    ):
        """
        Build daily actual annual revenue panel from lixinger financial_statements.

        Annual revenue = sum of single-quarter values Q1+Q2+Q3+Q4 for a fiscal year.
        ...
        # Sum 4 quarters per (symbol, fiscal_year) -> annual revenue
        annual_rev = (
            df.groupby(['symbol', 'fiscal_year'])[field]
            .sum()
            .reset_index()
            .rename(columns={field: 'annual_rev'})
        )
        ...
        merged = annual_rev.merge(q4_disclosure, on=['symbol', 'fiscal_year'], how='inner')
        ...
        # Pivot: rows=symbol, cols=disclosure_date, values=annual_rev
    ...
    def _load_forecast_rev_panel(
        ...
        op_rt values converted from 万元 to 元 (* 1e4).
        ...
        try:
            ...
            fc_df = pd.read_sql_query(
                f"""
                SELECT ts_code, report_date, quarter, op_rt
                FROM report_rc
            ...
            warnings.warn(f"REV_FG: failed to load tushare data: {exc}")
            return result
        ...
        # Convert 万元 -> 元
        fc_df['op_rt_yuan'] = fc_df['op_rt'].astype(float) * 1e4
        ...
        for t_idx, t in enumerate(dates_d):
            ...
            grp = fy1_window.groupby('code6')['op_rt_yuan']
...
# ------------------------------------------------------------------
# Smoke test (run: python rev_fg.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("REV_FG factor smoke test")
    print("=" * 60)
