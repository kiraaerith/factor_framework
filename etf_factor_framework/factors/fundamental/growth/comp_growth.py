"""
COMP_GROWTH factor (Composite Growth Rate via Linear Regression)

Computes:  g_comp = b / mean(x_{t-T..t})
...
Factor direction : positive  (higher composite growth is better)
Factor category  : growth - composite growth method
...
Notes:
  ...
  - Mean-zero denominator (|mean| < 1.0 yuan) yields NaN to avoid explosion.
  - Mean-negative with positive slope yields negative g_comp (sign inversion);
    users should be aware of this when field has negative-mean stocks.
...
FACTOR_NAME = "COMP_GROWTH"
FACTOR_DIRECTION = 1  # positive: higher composite growth is better

# Minimum absolute mean to avoid explosion near-zero denominator (1 yuan in lixinger unit)
MIN_ABS_MEAN = 1.0
...
def _compute_comp_growth_series(vals_arr: np.ndarray, rd_arr: np.ndarray, T: int) -> np.ndarray:
    """
    Compute COMP_GROWTH for one symbol's quarterly series.
    ...
    Returns:
        np.ndarray (n,) float64 - g_comp for each quarterly row
    ...
    for i in range(n):
        ...
        if np.isnan(mean_x) or np.isnan(b) or abs(mean_x) < MIN_ABS_MEAN:
            continue
...
class COMP_GROWTH(FundamentalFactorCalculator):
    """
    Composite Growth Rate factor using linear regression over T quarters.

    Computes: g_comp = b / mean(x_{t-T..t})
    where b is OLS slope over the T-quarter window.
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("COMP_GROWTH: no raw fundamental data available")
        ...
        if raw.empty:
            raise ValueError(
                f"COMP_GROWTH: no data for field '{self.field}' after filtering"
            )
        ...
        # --- Step 3: Compute COMP_GROWTH per symbol ---
        comp_growth_col = np.full(len(raw), np.nan)
        ...
        for symbol, grp in raw.groupby("symbol", sort=False):
            ...
            g_arr = _compute_comp_growth_series(vals_arr, rd_arr, self.T)
            comp_growth_col[idx] = g_arr

        raw["comp_growth"] = comp_growth_col

        # --- Step 4: Forward-fill to daily panel via report_date ---
        panel_df = _build_daily_panel(
            raw, "comp_growth", trading_dates, fundamental_data.end_date
        )
        if panel_df is None or panel_df.empty:
            raise ValueError(
                f"COMP_GROWTH: daily panel is empty for field '{self.field}'"
            )
        ...
        if len(symbols_arr) == 0:
            raise ValueError("COMP_GROWTH: no mainboard stocks after filtering")
...
# ------------------------------------------------------------------
# Smoke test  (run: python comp_growth.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("COMP_GROWTH factor smoke test")
    print("  field=q_ps_toi_c  T=8  (revenue 2-year composite growth)")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute COMP_GROWTH factor")
    calculator = COMP_GROWTH(field="q_ps_toi_c", T=8)
