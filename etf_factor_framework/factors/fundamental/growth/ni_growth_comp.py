"""
NI_GROWTH_COMP factor (Net Income Growth Composite)

Computes a composite of 8 sub-factors, all based on attributable net profit
(q_ps_npatoshopc_c) and its YoY growth rate (q_ps_npatoshopc_c_y2y):
...
Factor direction : positive (higher composite NI growth is better)
Factor category  : growth - NI composite multi-method

Notes:
  - PCT_GROWTH denominator = |x_{t-T}|; near-zero (<1 yuan) -> NaN.
  - SURPRISE_GROWTH requires >= 4 valid historical quarters; std=0 -> NaN.
  - ACCEL uses q_ps_npatoshopc_c_y2y winsorized to [-5, 5];
...
FACTOR_NAME = "NI_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive

# Fields
NI_FIELD = "q_ps_npatoshopc_c"          # single-quarter attributable NI
NI_Y2Y_FIELD = "q_ps_npatoshopc_c_y2y"  # pre-computed YoY growth rate
...
def _compute_pct_growth_panel(
    ...
    """
    Compute PCT_GROWTH = (x_t - x_{t-T}) / |x_{t-T}| for NI_FIELD over T quarters.
    ...
    x_t = qdf[NI_FIELD].values.astype(float)
...
def _compute_accel_panel(
    ...
    """
    Compute ACCEL sub-factor for q_ps_npatoshopc_c_y2y with window T.
    """
    if qdf_y2y.empty:
        return None
    ...
    qdf_y2y[NI_Y2Y_FIELD] = qdf_y2y[NI_Y2Y_FIELD].clip(
    ...
    for symbol, grp in qdf_y2y.groupby("symbol", sort=False):
        ...
        g_arr = grp_sorted[NI_Y2Y_FIELD].values.astype(float)
...
def _compute_robust_growth_panel(
    ...
    """Compute ROBUST_GROWTH sub-factor for NI_FIELD with window T."""
    if qdf.empty:
        return None
    ...
    for symbol, grp in qdf.groupby("symbol", sort=False):
        grp_sorted = grp.sort_values("date")
        idx = grp_sorted.index
        vals = grp_sorted[NI_FIELD].values.astype(float)
        rd_arr = grp_sorted["report_date"].values.astype("datetime64[ns]")
...
class NI_GROWTH_COMP(FundamentalFactorCalculator):
    """
    Net Income Growth Composite factor.

    Combines 8 sub-factors for attributable net profit growth:
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Compute NI_GROWTH_COMP daily panel (N_stocks x T_days).

        Steps:
          1. Load raw fundamental data.
          2. Build deduplicated quarterly panels for NI_FIELD and NI_Y2Y_FIELD.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("NI_GROWTH_COMP: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Step 2: Build deduplicated quarterly panels
        qdf_ni = _build_quarterly_panel(raw, NI_FIELD)
        ...
        # PCT_GROWTH: T=2, 4, 20
        for T in [2, 4, 20]:
            panel = _compute_pct_growth_panel(qdf_ni, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
                sub_names.append(f"pct_T{T}")

        # SURPRISE_GROWTH: T=8
        panel = _compute_surprise_growth_panel(qdf_ni, 8, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
            sub_names.append("surprise_T8")

        # ACCEL: T=4, 20 (uses NI_Y2Y_FIELD)
        if not qdf_y2y.empty:
            ...
            warnings.warn(
                f"NI_GROWTH_COMP: field '{NI_Y2Y_FIELD}' not available; "
                "ACCEL sub-factors skipped."
            )

        # ROBUST_GROWTH: T=4, 20
        for T in [4, 20]:
            panel = _compute_robust_growth_panel(qdf_ni, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
        ...
        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"NI_GROWTH_COMP: only {len(sub_panels)} valid sub-panels "
        ...
        if len(symbols_arr) == 0:
            raise ValueError("NI_GROWTH_COMP: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"NI_GROWTH_COMP NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python ni_growth_comp.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("NI_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute NI_GROWTH_COMP factor")
    calculator = NI_GROWTH_COMP()
