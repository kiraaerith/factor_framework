"""
NA_GROWTH_COMP factor (Net Assets Growth Composite)

Computes a composite of 2 sub-factors based on net assets / shareholders equity
(q_bs_toe_t):
...
Data source : lixinger.financial_statements  (q_bs_toe_t)
Factor direction : positive (higher composite net assets growth is better)
Factor category  : growth - capital investment dimension

Design rationale:
  Net assets (shareholders equity) is a balance-sheet time-point value; its
  series is smoother than income-statement flow items. PCT_GROWTH and ACCEL
  add little incremental information on smooth series, so NA_GROWTH_COMP uses
  only SURPRISE_GROWTH (deviation-from-mean for recency signal) and
  ROBUST_GROWTH (long-run delta-per-unit-volatility for sustained expansion).

Notes:
  - q_bs_toe_t can be negative (accumulated losses / leverage); this is valid
    business data and should NOT be filtered out. Negative net assets still
    carry meaningful growth signals.
...
FACTOR_NAME = "NA_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive

# Field
NA_FIELD = "q_bs_toe_t"  # net assets / shareholders equity, balance sheet time-point
...
def _build_quarterly_panel(raw: pd.DataFrame, field: str) -> pd.DataFrame:
    ...
    Note: net assets (q_bs_toe_t) can be negative; no positivity filter applied.
    """
...
def _compute_surprise_growth_panel(
    ...
    Uses T quarterly lags (k=1..T). PIT check: lag report_date <= current report_date.
    ...
    x_t = qdf[NA_FIELD].values.astype(float)
...
def _compute_robust_growth_panel(
    ...
    window_vals[:, T] = qdf[NA_FIELD].values.astype(float)
...
class NA_GROWTH_COMP(FundamentalFactorCalculator):
    """
    Net Assets Growth Composite factor.

    Combines 2 sub-factors for net assets / shareholders equity (q_bs_toe_t) growth:
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Steps:
          ...
          2. Build deduplicated quarterly panel for NA_FIELD (q_bs_toe_t).
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("NA_GROWTH_COMP: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Step 2: Build deduplicated quarterly panel (no positivity filter for net assets)
        qdf_na = _build_quarterly_panel(raw, NA_FIELD)

        if qdf_na.empty:
            raise ValueError(f"NA_GROWTH_COMP: no valid {NA_FIELD} data after filtering")

        # Step 3: Compute sub-factor daily panels
        sub_panels = []
        sub_names = []

        panel = _compute_surprise_growth_panel(qdf_na, T_SURPRISE, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
            sub_names.append(f"surprise_T{T_SURPRISE}")

        panel = _compute_robust_growth_panel(qdf_na, T_ROBUST, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
        ...
        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"NA_GROWTH_COMP: only {len(sub_panels)} valid sub-panels "
        ...
        if len(symbols_arr) == 0:
            raise ValueError("NA_GROWTH_COMP: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"NA_GROWTH_COMP NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python na_growth_comp.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("NA_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute NA_GROWTH_COMP factor")
    calculator = NA_GROWTH_COMP()
