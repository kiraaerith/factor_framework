"""
OP_GROWTH_COMP factor (Operating Profit Growth Composite)

Computes a composite of 8 sub-factors, all based on single-quarter operating
profit (q_ps_op_c) and its YoY growth rate (q_ps_op_c_y2y):
...
Factor direction : positive (higher composite OP growth is better)
Factor category  : growth - OP composite multi-method

Notes:
  - PCT_GROWTH denominator = |x_{t-T}|; near-zero (<1 yuan) -> NaN.
  - SURPRISE_GROWTH requires >= 4 valid historical quarters; std=0 -> NaN.
  - ACCEL uses q_ps_op_c_y2y winsorized to [-5, 5];
  ...
  - OP_GROWTH_COMP is a standalone factor and is NOT included in multi-indicator
    composites (COMP_GROWTH etc.) to avoid over-weighting NI-correlated signals.
"""
...
FACTOR_NAME = "OP_GROWTH_COMP"
FACTOR_DIRECTION = 1  # positive

# Fields
OP_FIELD = "q_ps_op_c"          # single-quarter operating profit
OP_Y2Y_FIELD = "q_ps_op_c_y2y"  # pre-computed YoY growth rate
...
def _compute_pct_growth_panel(
    ...
    """
    Compute PCT_GROWTH = (x_t - x_{t-T}) / |x_{t-T}| for OP_FIELD over T quarters.
    ...
    x_t = qdf[OP_FIELD].values.astype(float)
...
def _compute_accel_panel(
    ...
    """
    Compute ACCEL sub-factor for q_ps_op_c_y2y with window T.
    """
    if qdf_y2y.empty:
        return None

    qdf_y2y = qdf_y2y.copy()
    qdf_y2y[OP_Y2Y_FIELD] = qdf_y2y[OP_Y2Y_FIELD].clip(
    ...
    for symbol, grp in qdf_y2y.groupby("symbol", sort=False):
        ...
        g_arr = grp_sorted[OP_Y2Y_FIELD].values.astype(float)
...
def _compute_robust_growth_panel(
    ...
    """Compute ROBUST_GROWTH sub-factor for OP_FIELD with window T."""
    if qdf.empty:
        return None
    ...
    for symbol, grp in qdf.groupby("symbol", sort=False):
        grp_sorted = grp.sort_values("date")
        idx = grp_sorted.index
        vals = grp_sorted[OP_FIELD].values.astype(float)
        rd_arr = grp_sorted["report_date"].values.astype("datetime64[ns]")
...
class OP_GROWTH_COMP(FundamentalFactorCalculator):
    """
    Operating Profit Growth Composite factor.

    Combines 8 sub-factors for operating profit growth:
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Compute OP_GROWTH_COMP daily panel (N_stocks x T_days).

        Steps:
          1. Load raw fundamental data.
          2. Build deduplicated quarterly panels for OP_FIELD and OP_Y2Y_FIELD.
        ...
        if fundamental_data._raw_data is None or fundamental_data._raw_data.empty:
            raise ValueError("OP_GROWTH_COMP: no raw fundamental data available")

        raw = fundamental_data._raw_data

        # Step 2: Build deduplicated quarterly panels
        qdf_op = _build_quarterly_panel(raw, OP_FIELD)
        qdf_y2y = _build_quarterly_panel(raw, OP_Y2Y_FIELD) if OP_Y2Y_FIELD in raw.columns else pd.DataFrame()
        ...
        # PCT_GROWTH: T=2, 4, 20
        for T in [2, 4, 20]:
            panel = _compute_pct_growth_panel(qdf_op, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
                sub_names.append(f"pct_T{T}")

        # SURPRISE_GROWTH: T=8
        panel = _compute_surprise_growth_panel(qdf_op, 8, trading_dates, end_date)
        if panel is not None and not panel.empty:
            sub_panels.append(panel)
            sub_names.append("surprise_T8")

        # ACCEL: T=4, 20 (uses OP_Y2Y_FIELD)
        if not qdf_y2y.empty:
            ...
            warnings.warn(
                f"OP_GROWTH_COMP: field '{OP_Y2Y_FIELD}' not available; "
                "ACCEL sub-factors skipped."
            )

        # ROBUST_GROWTH: T=4, 20
        for T in [4, 20]:
            panel = _compute_robust_growth_panel(qdf_op, T, trading_dates, end_date)
            if panel is not None and not panel.empty:
                sub_panels.append(panel)
        ...
        if len(sub_panels) < MIN_VALID_SUBS:
            raise ValueError(
                f"OP_GROWTH_COMP: only {len(sub_panels)} valid sub-panels "
        ...
        if len(symbols_arr) == 0:
            raise ValueError("OP_GROWTH_COMP: no mainboard stocks after filtering")

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(f"OP_GROWTH_COMP NaN ratio is high: {nan_ratio:.1%}")
...
# ---------------------------------------------------------------------------
# Smoke test  (run: python op_growth_comp.py)
# ---------------------------------------------------------------------------
if __name__ == "__main__":
    warnings.filterwarnings("ignore")

    print("=" * 60)
    print("OP_GROWTH_COMP factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute OP_GROWTH_COMP factor")
    calculator = OP_GROWTH_COMP()
