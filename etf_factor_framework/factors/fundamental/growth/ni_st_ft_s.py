"""
NI_ST_FT_S factor (Net Income Short-Term Forecast Trend - Simplified)

Simplified post-processing version of NI_ST_FT. Core computation is identical:
  NI_ST_FT_1Q = (FY1_consensus(t) - FY1_consensus(t-65))  / |FY1_consensus(t-65)|
  NI_ST_FT_6M = (FY1_consensus(t) - FY1_consensus(t-130)) / |FY1_consensus(t-130)|

Composite (simplified post-processing vs NI_ST_FT):
  - Winsorize each window to [-1, 5] (no additional ±3σ step)
  - Cross-section rank normalize (0~1) per window
  - Equal-weight composite: 0.5 * rank_1Q + 0.5 * rank_6M
  - Final winsorize to [5th pct, 95th pct] per cross-section
  - No market cap / industry neutralization, no standardization

FY1 definition:
  - FY1 = current calendar year (t.year), always.
  - For growth rate, t and t-window both use t.year as FY1 target.
...
Factor direction: positive (upward revision of earnings forecasts is better)
...
def _winsorize_panel_bound(panel: np.ndarray, lower: float, upper: float) -> np.ndarray:
    """
    Winsorize each cross-section to [lower, upper] (hard clip only, no sigma step).
    panel: (N, T) float64, NaN-safe.
    Returns a new (N, T) array.
    """
    out = panel.copy()
    T = out.shape[1]
    # Loop: ~T times (number of trading days, e.g. ~2000 for 10yr panel)
    for t in range(T):
        cs = out[:, t]
        valid_mask = ~np.isnan(cs)
        if valid_mask.sum() < 2:
            continue
        out[valid_mask, t] = np.clip(cs[valid_mask], lower, upper)
    return out


def _winsorize_panel_quantile(panel: np.ndarray, q_low: float, q_high: float) -> np.ndarray:
    """
    Winsorize each cross-section to [q_low, q_high] quantile bounds.
    panel: (N, T) float64, NaN-safe.
    Returns a new (N, T) array.
    """
    out = panel.copy()
    T = out.shape[1]
    # Loop: ~T times (number of trading days)
    for t in range(T):
        cs = out[:, t]
        valid_mask = ~np.isnan(cs)
        if valid_mask.sum() < 2:
        ...
        lo = np.percentile(valid_vals, q_low * 100)
        hi = np.percentile(valid_vals, q_high * 100)
        out[valid_mask, t] = np.clip(valid_vals, lo, hi)
    return out
...
class NI_ST_FT_S(FundamentalFactorCalculator):
    """
    Net Income Short-Term Forecast Trend - Simplified factor.

    Same core as NI_ST_FT but with simplified post-processing:
    no ±3σ winsorization, no neutralization, no standardization.
    Final step: quantile winsorize composite to [5%, 95%].
    """
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """Compute NI_ST_FT_S daily panel."""
        fd = fundamental_data
        fd._load_raw_data()
        ...
        # Step 1: Build mainboard symbol universe from tushare report_rc
        symbols = self._get_mainboard_symbols()
        N = len(symbols)

        if N == 0:
            raise ValueError("NI_ST_FT_S: no mainboard symbols found in tushare report_rc")
        ...
        for t_idx in range(T):
            ...
            if t0 >= 0:
                ...
                growth_1q[:, t_idx] = np.where(bad, np.nan, g)
            ...
            if t0 >= 0:
                ...
                with np.errstate(divide='ignore', invalid='ignore'):
                    g = (curr - prev) / np.abs(prev)
                bad = (prev == 0) | np.isnan(prev) | np.isnan(curr) | np.isinf(g)
                growth_6m[:, t_idx] = np.where(bad, np.nan, g)

        # Step 4: Winsorize each window to [-1, 5] (no ±3σ step in simplified version)
        growth_1q = _winsorize_panel_bound(growth_1q, lower=-1.0, upper=5.0)
        growth_6m = _winsorize_panel_bound(growth_6m, lower=-1.0, upper=5.0)

        # Step 5: Cross-section rank normalization (0~1) per window
        rank_1q = _cross_section_rank(growth_1q)
        rank_6m = _cross_section_rank(growth_6m)

        # Step 6: Equal-weight composite
        ...
        # Step 7: Winsorize composite to [5%, 95%] per cross-section
        composite = _winsorize_panel_quantile(composite, q_low=0.05, q_high=0.95)

        nan_ratio = np.isnan(composite).mean()
        if nan_ratio > 0.8:
            warnings.warn(
                f"NI_ST_FT_S: NaN ratio is high ({nan_ratio:.1%}). "
    ...
    def _get_mainboard_symbols(self) -> np.ndarray:
        """
        Extract mainboard symbols from tushare report_rc.

        Querying tushare directly gives a stable universe independent of
        fd.end_date, avoiding false leakage signals from universe differences.
        """
        try:
            ...
            warnings.warn(f"NI_ST_FT_S: cannot load tushare symbols: {exc}")
    ...
    def _build_consensus_panel(
        ...
        try:
            ...
            warnings.warn(f"NI_ST_FT_S: failed to load tushare data: {exc}")
            return result
        ...
        # Sort for deterministic results regardless of SQL row order
        ...
        # Loop: ~T times (number of trading days, e.g. ~3000 for full panel)
        for t_idx, t in enumerate(dates_d):
            ...
            # Per (code6, org_name), keep only the most recent report.
            latest_per_org = (
                fy1_window
            ...
            # Consensus: median of orgs' latest np, require >= MIN_ANALYST_COUNT
...
# ------------------------------------------------------------------
# Smoke test (run: python ni_st_ft_s.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("NI_ST_FT_S factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute NI_ST_FT_S factor")
    calculator = NI_ST_FT_S()
