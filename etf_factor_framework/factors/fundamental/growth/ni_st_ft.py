"""
NI_ST_FT factor (Net Income Short-Term Forecast Trend)

Computes the revision momentum of analyst FY1 net profit consensus:
  ...
  NI_ST_FT = 0.5 * rank(NI_ST_FT_1Q) + 0.5 * rank(NI_ST_FT_6M)
...
class NI_ST_FT(FundamentalFactorCalculator):
    """
    Net Income Short-Term Forecast Trend factor.

    Measures the momentum of analyst FY1 net profit consensus revisions
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        ...
        # Step 1: Build mainboard symbol universe from lixinger
        symbols = self._get_mainboard_symbols(fd)
        N = len(symbols)

        if N == 0:
            raise ValueError("NI_ST_FT: no mainboard symbols found in lixinger data")
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"NI_ST_FT: NaN ratio is high ({nan_ratio:.1%}). "
    ...
    def _get_mainboard_symbols(self, fd: FundamentalData) -> np.ndarray:
        ...
        try:
            ...
            warnings.warn(f"NI_ST_FT: cannot load tushare symbols: {exc}")
    ...
    def _build_consensus_panel(
        ...
        try:
            ...
            warnings.warn(f"NI_ST_FT: failed to load tushare data: {exc}")
            return result
...
# ------------------------------------------------------------------
# Smoke test (run: python ni_st_ft.py)
# ------------------------------------------------------------------
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("NI_ST_FT factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute NI_ST_FT factor")
    calculator = NI_ST_FT()
