"""
EQUAL_RANK_POOL18_INDNEU: Equal-weight rank composite of 18-factor pool,
with industry neutralization applied to each sub-factor before ranking.

Pipeline:
  1. Compute 18 sub-factors, align to common symbols/dates
  2. Industry neutralize each sub-factor (行业内 z-score)
  3. Cross-sectional percentile rank [0,1]
  4. Flip rank for negative-direction factors
  5. Composite = mean of valid ranks (require >= 6 of 18)
...
FACTOR_NAME = "EQUAL_RANK_POOL18_INDNEU"
FACTOR_DIRECTION = 1
...
class EQUAL_RANK_POOL18_INDNEU(FundamentalFactorCalculator):
    """Equal-weight rank composite with industry neutralization."""
    ...
    def params(self) -> dict:
        return {
            ...
            "method": "equal_rank",
            "preprocessing": ["industry_neutral"],
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        # Step 3: align all sub-factors into (N, T, F)
        features_nt = np.full((N, T, F), np.nan)
        for f_idx, (fd, direction, cls_name) in enumerate(sub_results):
            ...
            features_nt[:, :, f_idx] = fd.values[np.ix_(sym_indices, date_indices)]

        # Step 4: industry neutralization via FactorPreprocessor
        logger.info(f"{FACTOR_NAME}: applying industry neutralization...")
        industry_map = fundamental_data.get_industry_map()

        preprocessor = FactorPreprocessor(
            steps=['industry_neutral'],
        )
        preprocessor.set_industry_map(industry_map)

        features_nt = preprocessor.transform(features_nt, common_symbols)
        logger.info(f"{FACTOR_NAME}: industry neutralization done")

        # Step 5: rank, flip, accumulate
        rank_sum = np.zeros((N, T), dtype=np.float64)
        valid_count = np.zeros((N, T), dtype=np.float64)

        for f_idx, (_, direction, cls_name) in enumerate(sub_results):
            ranked = _cross_sectional_rank(features_nt[:, :, f_idx])
        ...
        # Step 6: average (require >= MIN_VALID_FACTORS valid)
        composite = np.where(
