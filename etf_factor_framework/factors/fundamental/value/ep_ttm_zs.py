"""
EP_TTM_ZS factor (EP TTM Time-Series Z-Score, Purified from 6-Month Reversal)

Formula:
  1. EP_TTM = 1 / pe_ttm  (daily, from lixinger.fundamental)
  2. ZS_t = (EP_TTM_t - mean(EP_TTM, window=126)) / std(EP_TTM, window=126)
             per stock, rolling 126-day window, min_periods=63
  3. REV_6M_t = close_t / close_{t-126} - 1  (from juejin ohlcv_adjusted)
  4. EP_TTM_ZS = residual of cross-section OLS: ZS_t ~ z-score(REV_6M_t)

Data fields:
  - pe_ttm  : lixinger.fundamental (daily valuation)
  - close   : juejin ohlcv_adjusted (daily adjusted close price)

Factor direction: positive (higher ZS = current EP elevated above historical mean,
                             net of reversal = better mean-reversion opportunity)
...
FACTOR_NAME = "EP_TTM_ZS"
FACTOR_DIRECTION = 1    # positive: higher ZS = current EP elevated vs history = cheaper
MIN_ABS_PE = 1e-6       # near-zero PE threshold for NaN assignment
ZS_WINDOW = 126         # rolling window in trading days (~6 months)
ZS_MIN_PERIODS = 63     # minimum non-NaN observations for Z-Score (half window)
...
class EP_TTM_ZS(FundamentalFactorCalculator):
    """
    EP TTM Time-Series Z-Score factor, purified from 6-month reversal.

    Steps:
      1. EP_TTM = 1/pe_ttm (lixinger.fundamental, daily)
      2. ZS = 126-day rolling time-series Z-Score of EP_TTM, per stock (min_periods=63)
      3. REV_6M = 126-trading-day cumulative return (juejin ohlcv_adjusted)
      4. EP_TTM_ZS = residual of cross-section OLS: ZS ~ z-score(REV_6M), per day
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Calculate EP_TTM_ZS daily panel.
        ...
        # ------------------------------------------------------------------
        # Step 2: Rolling 126-day time-series Z-Score per stock
        # DataFrame shape: (T, N) so rolling() works column-wise
        # ------------------------------------------------------------------
        ep_df = pd.DataFrame(ep_values.T, index=trading_dates, columns=symbols)

        roll = ep_df.rolling(window=ZS_WINDOW, min_periods=ZS_MIN_PERIODS)
        roll_mean = roll.mean()
        roll_std = roll.std(ddof=1)

        # Avoid division by zero (std == 0)
        roll_std_safe = roll_std.replace(0.0, np.nan)

        zs_df = (ep_df - roll_mean) / roll_std_safe
        ...
        if not price_df.empty:
            ...
            # Align to symbols and trading_dates
            rev6m_aligned = (
                rev6m_panel
        ...
        # ------------------------------------------------------------------
        # Step 4: Cross-section OLS per day: ZS ~ z-score(REV_6M), residuals
        # ------------------------------------------------------------------
        ...
        for t in range(T):
            ...
            # Assign residuals; invalid positions remain NaN
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"EP_TTM_ZS NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python ep_ttm_zs.py)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("EP_TTM_ZS factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute EP_TTM_ZS factor")
    calculator = EP_TTM_ZS()
