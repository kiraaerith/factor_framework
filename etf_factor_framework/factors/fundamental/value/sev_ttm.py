"""
SEV_TTM factor (Sales to Enterprise Value, TTM)

Formula: SEV_TTM = Revenue_TTM / EV
  - Revenue_TTM = mc / ps_ttm
      mc     : lixinger.fundamental daily total market cap (yuan)
      ps_ttm : lixinger.fundamental daily PS-TTM (price/sales ratio, unitless)
      => Revenue_TTM = mc / ps_ttm  (yuan, TTM revenue)
  - EV = mc + TL + MI - Cash
...
Factor direction: positive (higher SEV_TTM = cheaper on revenue/EV basis)
Factor category: value - denominator-improved value

Note on units: lixinger.fundamental stores mc in yuan (CNY), NOT in yi yuan (1e8).
...
FACTOR_NAME = "SEV_TTM"
FACTOR_DIRECTION = 1   # positive: higher SEV_TTM (cheaper on revenue/EV basis) is better
MIN_ABS_PS = 1e-6      # |ps_ttm| below this => near-zero revenue => NaN
...
class SEV_TTM(FundamentalFactorCalculator):
    """
    Sales to Enterprise Value (TTM) factor.

    SEV_TTM = Revenue_TTM / EV
    where:
      Revenue_TTM = mc (yuan) / ps_ttm  (TTM sales from market-implied PS ratio)
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Calculate SEV_TTM daily panel.

        Returns:
            FactorData: N stocks x T days, values = Revenue_TTM / EV (float64)
        """
        # --- 1. Load daily valuation panels from lixinger.fundamental ---
        mc_v, mc_s, mc_d = fundamental_data.get_valuation_panel("mc")
        ps_v, ps_s, ps_d = fundamental_data.get_valuation_panel("ps_ttm")

        if mc_v.size == 0:
            raise ValueError("SEV_TTM: get_valuation_panel('mc') returned empty array")
        if ps_v.size == 0:
            raise ValueError("SEV_TTM: get_valuation_panel('ps_ttm') returned empty array")
        ...
        # --- 3. Determine universe: intersection of mc and ps_ttm (both needed for Revenue_TTM) ---
        #        then apply mainboard filter
        mc_s_sorted = np.sort(mc_s)
        ps_s_sorted = np.sort(ps_s)

        base_syms = np.intersect1d(mc_s_sorted, ps_s_sorted)
        mainboard_mask = np.array([_is_mainboard(s) for s in base_syms])
        common_syms = base_syms[mainboard_mask]

        if len(common_syms) == 0:
            raise ValueError("SEV_TTM: no mainboard stocks in mc ∩ ps_ttm universe")

        # common dates = mc_d (trading calendar; all valuation panels share it)
        common_dates = mc_d

        # --- 4. Align mc and ps_ttm to (common_syms, common_dates) ---
        mc_s_sorted_idx = np.argsort(mc_s)
        mc_s_s = mc_s[mc_s_sorted_idx]
        mc_v_s = mc_v[mc_s_sorted_idx]

        ps_s_sorted_idx = np.argsort(ps_s)
        ps_s_s = ps_s[ps_s_sorted_idx]
        ps_v_s = ps_v[ps_s_sorted_idx]

        mc_aligned = _align_to(mc_v_s, mc_s_s, mc_d, common_syms, common_dates)
        ps_aligned = _align_to(ps_v_s, ps_s_s, ps_d, common_syms, common_dates)

        # --- 5. Align quarterly panels (already forward-filled to daily in get_daily_panel) ---
        tl_s_sorted_idx = np.argsort(tl_s)
        tl_aligned = _align_to(tl_v[tl_s_sorted_idx], tl_s[tl_s_sorted_idx], tl_d,
                                common_syms, common_dates)

        mi_s_sorted_idx = np.argsort(mi_s)
        mi_aligned = _align_to(mi_v[mi_s_sorted_idx], mi_s[mi_s_sorted_idx], mi_d,
                                common_syms, common_dates)

        cash_s_sorted_idx = np.argsort(cash_s)
        cash_aligned = _align_to(cash_v[cash_s_sorted_idx], cash_s[cash_s_sorted_idx], cash_d,
                                  common_syms, common_dates)

        # --- 6. Compute Revenue_TTM = mc / ps_ttm ---
        with np.errstate(divide="ignore", invalid="ignore"):
            rev_ttm = np.where(
                np.abs(ps_aligned) >= MIN_ABS_PS,
                mc_aligned / ps_aligned,
                np.nan,
            )
        # Revenue should be positive; reject negative/zero
        rev_ttm = np.where(rev_ttm > 0, rev_ttm, np.nan)
        ...
        # --- 8. Compute SEV_TTM = Revenue_TTM / EV ---
        with np.errstate(divide="ignore", invalid="ignore"):
            sev = np.where(
                (ev >= MIN_EV) & ~np.isnan(rev_ttm) & ~np.isnan(tl_aligned),
                rev_ttm / ev,
                np.nan,
            )
        sev = sev.astype(np.float64)

        # Replace any remaining inf
        sev = np.where(np.isinf(sev), np.nan, sev)

        nan_ratio = np.isnan(sev).mean() if sev.size > 0 else 1.0
        if nan_ratio > 0.8:
            warnings.warn(
                f"SEV_TTM NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python sev_ttm.py)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("SEV_TTM factor smoke test")
    print("=" * 60)
