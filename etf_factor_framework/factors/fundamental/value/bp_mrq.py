"""
BP_MRQ factor (Book-to-Price, Most Recent Quarter)
...
Factor direction: positive (higher BP = cheaper valuation = better)
...
Notes:
  ...
  - Stocks with pb < 0 (negative book value) yield BP_MRQ < 0, kept as-is.
...
FACTOR_NAME = "BP_MRQ"
FACTOR_DIRECTION = 1   # positive: higher BP (cheaper valuation) is better
...
class BP_MRQ(FundamentalFactorCalculator):
    """
    Book-to-Price (MRQ) factor.

    Computes BP_MRQ = 1 / pb for each stock on each trading day.
    Uses lixinger.fundamental.pb (daily, based on MRQ book value).
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Calculate BP_MRQ daily panel.

        Returns:
            FactorData: N stocks x T days, values = 1/pb (float64)
        """
        ...
        bp_values = np.where(np.isinf(bp_values), np.nan, bp_values)
        bp_values = bp_values.astype(np.float64)

        nan_ratio = np.isnan(bp_values).mean() if bp_values.size > 0 else 1.0
        if nan_ratio > 0.8:
            warnings.warn(
                f"BP_MRQ NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python bp_mrq.py)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("BP_MRQ factor smoke test")
    print("=" * 60)
    ...
    # Sanity: check known stocks have positive BP_MRQ
