"""
RDBP_MRQ factor (Research-Adjusted Book-to-Price, Most Recent Quarter)

Formula:
  KC_MRQ         = sum_{i=0}^{N} RD_quarter_i * 0.9^(i/4)   (i=0 is most recent quarter)
  adj_equity_mrq = q_bs_tetoshopc_t + KC_MRQ
  RDBP_MRQ       = adj_equity_mrq / mc
...
Factor category: value - improved value (R&D adjusted)

Notes:
  - q_ps_rade_c NaN => treated as 0 (firm has no R&D, knowledge capital = 0)
  - q_ps_rade_c negative => clipped to 0 (data anomaly)
  - q_bs_tetoshopc_t NaN => RDBP_MRQ = NaN
  - mc = 0 or NaN => RDBP_MRQ = NaN
  - Both mc and adj_equity are in yuan; ratio is dimensionless
  - Extended R&D history: 12 years before start_date to accumulate KC properly
...
FACTOR_NAME = "RDBP_MRQ"
FACTOR_DIRECTION = 1    # positive: higher adj book-to-price is better
N_QUARTERS = 40         # look-back window for KC accumulation
ANNUAL_DECAY = 0.90     # annual R&D depreciation rate
MIN_MC = 1e-6           # mc below this => NaN
...
def _compute_kc_info_consistent(
    rd_vals: np.ndarray,
    ...
    """
    Compute knowledge capital for each quarter in a single stock's R&D series,
    sorted by report_date order for information consistency.

    At position i (report_date order), only contributions from j <= i are included,
    ensuring no future data leakage even for late-filed quarterly reports.
    Decay is based on actual quarter distance (year*4 + quarter_num), not
    sequential index, correctly handling gaps and out-of-order filings.

    Args:
        rd_vals: 1-D array of R&D values sorted by report_date (oldest filing first),
                 already clipped to >= 0.
        quarter_idxs: 1-D int array of quarter indices (year*4 + quarter_num 0-3),
                 corresponding to each row's period_end date.
        n_quarters: maximum look-back window (in quarters).
        annual_decay: annual depreciation rate (e.g. 0.90).

    Returns:
        kc_vals: 1-D array of KC values, same length as rd_vals.
    """
    n_total = len(rd_vals)
    kc_vals = np.zeros(n_total, dtype=np.float64)
    for i in range(n_total):
        current_q = quarter_idxs[i]
        q_back = current_q - quarter_idxs[:i + 1]   # vectorized over j <= i
        valid = (q_back >= 0) & (q_back < n_quarters)
        if valid.any():
            decay = annual_decay ** (q_back[valid] / 4.0)
            kc_vals[i] = float(np.dot(rd_vals[:i + 1][valid], decay))
    return kc_vals


class RDBP_MRQ(FundamentalFactorCalculator):
    """
    Research-Adjusted Book-to-Price (MRQ) factor.

    Adds accumulated (depreciated) R&D investment to book equity to
    correct for the systematic under-statement of intangible assets under
    Chinese GAAP.  Then divides by daily market cap to get a daily BP-like
    ratio.
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Calculate RDBP_MRQ daily panel.

        Returns:
            FactorData: N stocks x T days, values = adj_equity / mc (float64)
        """
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Step 2: Query extended R&D + equity history from lixinger
        #         Go back 12 years to accumulate up to 48 quarters of KC
        # ------------------------------------------------------------------
        rd_load_start = (
        ...
        # (a report cannot be filed before the accounting period ends)
        ...
        # ------------------------------------------------------------------
        # Step 4: Compute Knowledge Capital (KC) per quarter per stock
        #         Sort by report_date for information-consistent KC:
        #         only R&D data publicly available at the time of each report
        #         contributes to its KC, preventing leakage from late filings.
        # ------------------------------------------------------------------
        df_raw = df_raw.sort_values(["symbol", "report_date", "date"]).reset_index(drop=True)

        # Clean R&D: NaN -> 0, negative -> 0
        df_raw["rd_clean"] = df_raw["q_ps_rade_c"].fillna(0.0).clip(lower=0.0)

        # Quarter index: year*4 + quarter_num(0-3), used for decay computation
        dates_idx = pd.DatetimeIndex(df_raw["date"])
        df_raw["q_idx"] = (dates_idx.year * 4 + (dates_idx.month - 1) // 3).astype(np.int32)

        kc_all = np.zeros(len(df_raw), dtype=np.float64)
        idx_offset = 0
        for _sym, grp in df_raw.groupby("symbol", sort=False):
            rd_vals = grp["rd_clean"].values
            q_idxs = grp["q_idx"].values
            kc_vals = _compute_kc_info_consistent(rd_vals, q_idxs, N_QUARTERS, ANNUAL_DECAY)
            n = len(kc_vals)
        ...
        # ------------------------------------------------------------------
        # Step 5: Compute adj_equity = tetoshopc + KC (yuan)
        #         NaN tetoshopc => NaN adj_equity
        # ------------------------------------------------------------------
        tetoshopc = df_raw["q_bs_tetoshopc_t"].values.astype(np.float64)
        kc = df_raw["kc"].values
        adj_equity = np.where(np.isnan(tetoshopc), np.nan, tetoshopc + kc)
        ...
        # ------------------------------------------------------------------
        # Step 9: RDBP_MRQ = adj_equity / mc  (both in yuan)
        # ------------------------------------------------------------------
        ...
        rdbp = np.where(np.isinf(rdbp), np.nan, rdbp).astype(np.float64)

        nan_ratio = np.isnan(rdbp).mean() if rdbp.size > 0 else 1.0
        if nan_ratio > 0.8:
            warnings.warn(
                f"RDBP_MRQ NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python rdbp_mrq.py)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("RDBP_MRQ factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute RDBP_MRQ factor")
    calculator = RDBP_MRQ()
