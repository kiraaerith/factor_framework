"""
EPFWD factor (Expected Earnings-to-Price, Forward)

Designed as a forward-earnings-based value factor:
  EPFWD = FY1_net_profit / mc

FY1 (analyst consensus forward earnings) is NOT currently available in the
database. This implementation uses TTM net profit as a fallback:
  EPFWD_fallback = 1 / pe_ttm  (equivalent to net_profit_TTM / mc)

Data fields:
  - pe_ttm : PE-TTM (market cap / TTM net profit attributable to parent)
             lixinger.fundamental (daily)

Factor direction: positive (higher EPFWD = cheaper expected valuation = better)
Factor category: value - forward earnings (分子端改进价值因子)

Notes:
  - pe_ttm is daily data, so this is a daily-updated factor with no reporting delay.
  - Stocks with |pe_ttm| < 1e-6 (near-zero earnings) are set to NaN.
  - Loss-making stocks (pe_ttm < 0) yield EPFWD < 0, kept as-is.
  - Mainboard filter: SHSE.60xxxx and SZSE.00xxxx only.
  - When FY1 data becomes available (e.g., Wind consensus), the calculate()
    method should prefer FY1 and fall back to TTM for uncovered stocks.
"""
...
FACTOR_NAME = "EPFWD"
FACTOR_DIRECTION = 1   # positive: higher EPFWD (cheaper expected valuation) is better
...
class EPFWD(FundamentalFactorCalculator):
    """
    Expected Earnings-to-Price (Forward) factor.

    Currently uses TTM fallback: EPFWD = 1 / pe_ttm
    (equivalent to net_profit_TTM / total_market_cap).

    Uses lixinger.fundamental.pe_ttm (daily, already TTM-based).
    Only mainboard stocks (SHSE.60xxxx, SZSE.00xxxx) are included.
    Stocks with |pe_ttm| < 1e-6 are set to NaN (near-zero net profit).
    """
    ...
    def calculate(self, fundamental_data: FundamentalData) -> FactorData:
        """
        Calculate EPFWD daily panel.

        Returns:
            FactorData: N stocks x T days, values = 1/pe_ttm (float64)
        """
        ...
        pe_values = pe_values[mainboard_mask]
        symbols = symbols[mainboard_mask]

        # Compute EPFWD = 1 / pe_ttm (TTM fallback)
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", RuntimeWarning)
            epfwd_values = np.where(
        ...
        epfwd_values = np.where(np.isinf(epfwd_values), np.nan, epfwd_values)
        epfwd_values = epfwd_values.astype(np.float64)

        nan_ratio = np.isnan(epfwd_values).mean() if epfwd_values.size > 0 else 1.0
        if nan_ratio > 0.8:
            warnings.warn(
                f"EPFWD NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python epfwd.py)
...
if __name__ == "__main__":
    ...
    # Sanity: check known stocks have reasonable EPFWD values
