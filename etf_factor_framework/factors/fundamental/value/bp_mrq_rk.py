"""
BP_MRQ_RK factor (BP_MRQ 2-Year Time-Series Rank, Purified from 2-Year Reversal)

Formula:
  1. BP_MRQ = 1 / pb  (daily, from lixinger.fundamental)
     - pb < 0 => BP_MRQ < 0 (kept as-is)
     - |pb| < 1e-6 or NaN/inf => NaN
  2. RK_t = rolling 504-day ascending rank of BP_MRQ per stock / count_valid
             (time-series rank, per-stock, window=504 trading days ~2 years,
  ...
  4. BP_MRQ_RK = residual of cross-section OLS: RK_t ~ z-score(REV_2Y_t)

Data fields:
  - pb   : lixinger.fundamental (daily valuation)
  - close : juejin ohlcv_adjusted (daily adjusted close price)

Factor direction: positive (higher = BP historically high relative to own history, net of reversal)
...
FACTOR_NAME = "BP_MRQ_RK"
FACTOR_DIRECTION = 1      # positive: higher rank = cheaper BP historically = better
MIN_ABS_PB = 1e-6         # near-zero PB threshold for NaN assignment
RK_WINDOW = 504           # rolling window in trading days (~2 years)
RK_MIN_PERIODS = 252      # minimum non-NaN observations for rank (~1 year)
...
class BP_MRQ_RK(FundamentalFactorCalculator):
    """
    BP_MRQ 2-Year Time-Series Rank factor, purified from 2-year reversal.

    Steps:
      1. BP_MRQ = 1/pb (lixinger.fundamental, daily)
      2. RK = 504-day rolling time-series rank of BP_MRQ, per stock
      3. REV_2Y = 504-trading-day cumulative return (juejin ohlcv_adjusted)
      4. BP_MRQ_RK = residual of cross-section OLS: RK ~ z-score(REV_2Y), per day
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        """
        Calculate BP_MRQ_RK daily panel.
        ...
        # ------------------------------------------------------------------
        # Step 2: Rolling 504-day time-series rank per stock
        # DataFrame shape: (T, N) so rolling().rank() works column-wise
        # ------------------------------------------------------------------
        bp_df = pd.DataFrame(bp_values.T, index=trading_dates, columns=symbols)
        rk_df = bp_df.rolling(window=RK_WINDOW, min_periods=RK_MIN_PERIODS).rank(pct=True)
        ...
        # ------------------------------------------------------------------
        # Step 4: Cross-section OLS per day: RK ~ z-score(REV_2Y), residuals
        # ------------------------------------------------------------------
        ...
        if nan_ratio > 0.8:
            warnings.warn(
                f"BP_MRQ_RK NaN ratio is high: {nan_ratio:.1%}, please check data"
            )
...
# ------------------------------------------------------------------
# Smoke test (run: python bp_mrq_rk.py)
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("BP_MRQ_RK factor smoke test")
    print("=" * 60)
    ...
    print(f"\n[Step 2] Compute BP_MRQ_RK factor")
    calculator = BP_MRQ_RK()
