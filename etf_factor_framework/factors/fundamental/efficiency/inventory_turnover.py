"""
存货周转率因子（Inventory Turnover）

数据字段：lixinger financial_statements.q_m_i_tor_t
因子方向：正向（存货周转率越高越好 - 表示库存周转效率高）
因子类别：运营效率
...
class INVENTORY_TURNOVER(FundamentalFactorCalculator):
    """
    存货周转率因子

    使用 lixinger 的 q_m_i_tor_t（存货周转率 TTM），
    ...
    def calculate(self, fundamental_data: FundamentalData, pricevol_data=None) -> FactorData:
        ...
        Returns:
            FactorData: N股 × T日，值为 q_m_i_tor_t（周转次数，如 3.2 表示周转3.2次）
        """
        values, symbols, dates = fundamental_data.get_daily_panel(FIELD)

        if values.size == 0:
            raise ValueError(f"INVENTORY_TURNOVER 因子：get_daily_panel('{FIELD}') 返回空数组，"
                             "请检查 lixinger 数据库中是否有该字段数据")

        nan_ratio = np.isnan(values).sum() / values.size
        if nan_ratio > 0.8:
            import warnings
            warnings.warn(f"INVENTORY_TURNOVER 因子 NaN 比例偏高：{nan_ratio:.1%}，请检查数据")
...
# ------------------------------------------------------------------
# 测试（python inventory_turnover.py 直接运行）
# ------------------------------------------------------------------
